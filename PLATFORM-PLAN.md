...
- 7s (2026-05-18): surfaces Sales During Operation in Revenue Output Blocks 3 + 4 (per-asset and Project Total). Block 3 splits into 3a (Pre-Sales recognition vintage matrix, unchanged) + 3b (Recognition Summary per period: Pre-Sales / Sales During Operation / Total Revenue Recognised). Block 4 splits into 4a (Pre-Sales cash vintage matrix, unchanged) + 4b (Cash Summary per period: Pre-Sales / Sales During Operation / Total Cash Collected). Project Total section gets matching Pre/SDO/Total rows for both Recognition and Cash. Engine unchanged (post-sales recognition + cash already same-period since 7f); pure UI surfacing. AR + UR (Blocks 5 + 6) intentionally remain pre-sales-only by construction (SDO collects + recognises same period, no balance forms). Replicates across every Sell asset automatically (UI iterates over all of them).

Verifier `scripts/verify-revenue-rebuild.ts` 32/32 green at end of Pass 7s.
