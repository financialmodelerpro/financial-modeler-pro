...
- 7z (2026-05-18): pin the handover convention and make it visible. **Rule (re-affirmed):** PIT-at-handover lumps every pre-sales cohort at the LAST construction year (= `phaseStart + constructionPeriods - 1 - projectStart`), NOT the first operations year. For a phase with Construction 2026-2029 (cp=4) and Operations 2030-2039, handover resolves to **2029** and SDO sales in 2030 recognise in their own sale year (operating-sales convention, same period). Engine + resolver have always done this; some user-facing copy was implicit. Surface the resolved year explicitly: (1) "At handover" pill in the Inputs tab now reads "At handover (2029)" — shows the calendar year inline so an off-by-one in phase config is instantly visible; (2) Block 3 (Output) caption now spells out "Handover resolves to YYYY (last construction year)" when PIT-at-handover is active; (3) 5 new verifier tests (A2-1..A2-5) pin the convention against regressions — `resolveHandoverYear` returns idx 3 (2029) for the user's setup, pre-sales recognition lumps at idx 3, SDO recognises at idx 4 (2030), no spillover.

Verifier `scripts/verify-revenue-rebuild.ts` 37/37 green at end of Pass 7z.
...
**Sub-passes (all shipped 2026-05-18):**
- **8a (shipped):** Engine baseline. `src/core/calculations/revenue/hospitality.ts`. Math per project-axis period y: ARN[y] = keys × daysPerYear; ORN[y] = ARN[y] × clamp(occupancy[y]); ADR[y] = applyIndexation(startingADR, y, adrIndexation); Rooms[y] = ORN[y] × ADR[y]; Guests[y] = ORN[y] × guestsPerOccupiedRoom; F&B[y] + Other[y] driven by `AncillaryRevenueConfig` with 3 modes (`percent_of_rooms` | `per_guest` | `fixed_amount`); Total = Rooms + F&B + Other. Outputs zero outside [opsStartIdx, opsEndIdx]. F&B and Other revenue are independently configurable (one can be % of rooms while the other is per-guest, etc.). `guestsPerOccupiedRoom` defaults to 1.5; `daysPerYear` defaults to 365. Both ancillary modes accept optional `IndexationConfig` for rate / amount escalation. 22 verifier scenarios (H1-1..H6-3).
- **8b (shipped):** Resolver + Inputs UI. `asset.revenue.operate` field on Asset schema (mirrors HospitalityConfig minus engine-resolved fields). `resolveHospitalityConfig` in revenue-resolvers.ts: sums keys across `metric='units'` sub-units, derives opsStartIdx / opsEndIdx from phase windows, hands config to engine. `computeAllSellResults` now also walks Operate parents + every companion (companions are the operate side of a Sell + Manage parent regardless of their own strategy field), produces `byHospitalityAsset` Map + `hospitalityProjectTotals`. Module2Revenue.tsx Hospitality AssetCard variant: SubUnitReferenceStrip (Pass 7x), Starting ADR (AccountingNumberInput) + indexation pills (None / YoY / Step), Occupancy ramp via InlineProfileStrip across the operations window, Guests per occupied room night input (default 1.5), F&B section with 3 mode pills + value input that switches based on mode, Other Revenue section with same 3-mode structure, DSO input (default 30 days). All FAST blue.
...
**Final verifier:** `scripts/verify-revenue-rebuild.ts` 66/66 green (was 32/32 pre-Pass 7s; +9 Pass 7+8 surface improvements + 22 hospitality engine + 7 DSO AR).
