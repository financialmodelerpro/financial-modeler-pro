...
export default function CourseWatchPage() {
  ...
  // the YouTubePlayer's tracker on mount so cross-session watch_seconds
  // for analytics accumulates correctly across logins. Not used to gate
  // any UI -- Mark Complete unlocks on `videoEnded` regardless.
  ...
  // POST throttle bookkeeping. The tracker emits ~every 10s during play,
  // and we forward to the DB for analytics. The `lastPostedRef` keeps
  // ack-spam off the wire; not part of any gate.
      ...
      if (course) {
        ...
        if (currentSess) {
          ...
          if (watchRecord) {
            ...
            // the current session's playback (migration 146 fix). Used
            // for analytics fidelity; no UI consumes the merged seconds.
  ...
  // Handle Mark Complete click. Parent only renders the button when
  // `videoEnded` is true (driven by YT's PlayerState.ENDED, the
  // `currentTime >= duration - 1` tick fallback, or the PAUSED-at-tail
  // fallback inside YouTubePlayer). Persists to certification_watch_history.
   ...
   * unmount. Posts the merged interval snapshot to the DB so admin
   * Watch Progress + Platform Analytics get accurate watch_seconds /
   * watch_percentage / last_position numbers. The numbers do NOT gate
   * Mark Complete -- that's purely driven by `videoEnded`.
   *
   * `force=true` indicates a real close event (PAUSED / ENDED / BUFFERING
   * / unmount) -- bypass the throttle so the final partial interval
   * lands in the DB.
   */
