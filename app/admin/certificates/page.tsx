...
export default function AdminCertificatesPage() {
  ...
  const fetchCerts = useCallback(async () => {
    setLoading(true);
    try {
      const certsRes = await fetch('/api/admin/certificates/sync');
      if (certsRes.ok) {
      ...
      <div style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E8F0FB', padding: '24px', marginBottom: 28 }}>
          ...
          <div style={{ marginTop: 14, padding: '10px 14px', background: '#EFF6FF', borderRadius: 8, fontSize: 12, color: '#1D4ED8' }}>
            ...
            {' '}to position text fields on the PDF. Certificates then issue automatically the moment a student passes their final exam. If an inline issuance ever misses, use the{' '}
            <a href="/admin/training-hub/certificates" style={{ fontWeight: 700, color: '#1D4ED8' }}>Training Hub Certificates</a>
