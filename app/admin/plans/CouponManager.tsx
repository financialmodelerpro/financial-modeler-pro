 ...
 * CouponManager
 *
 * Coupon management (create / list / activate / deactivate / delete), relocated
 * from the removed /admin/pricing editor into the Plan Builder. Functionality is
 * unchanged: it talks to the same /api/admin/pricing/coupons endpoint. Coupons
 * wire to checkout later (Phase F).
 *
...
export function CouponManager() {
  ...
  const [form, setForm] = useState({ code: '', discount_type: 'percentage', discount_value: '20', max_uses: '100', expires_at: '' });
  ...
  const createCoupon = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) { showToast('Coupon created', 'success'); setShowForm(false); setForm({ code: '', discount_type: 'percentage', discount_value: '20', max_uses: '100', expires_at: '' }); await load(); }
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: 16, marginTop: 24 }} data-testid="coupon-manager">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#0D2E5A' }}>Coupon codes</div>
          <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>Discount codes for checkout (wired to checkout in a later phase).</div>
        </div>
      ...
      {showForm && (
        <div style={{ background: '#F9FAFB', borderRadius: 8, border: '1px solid #e5e7eb', padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 12 }}>
            ...
            <div><label style={LBL}>Value</label><input type="number" style={INP} value={form.discount_value} onChange={(e) => setForm((f) => ({ ...f, discount_value: e.target.value }))} /></div>
          ...
          <button onClick={createCoupon} disabled={saving || !form.code.trim()} data-testid="coupon-create"
            style={{ padding: '8px 18px', borderRadius: 6, border: 'none', background: '#2EAA4A', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Create coupon</button>
        </div>
      )}
      ...
      <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {coupons.length === 0 ? (
              <tr><td colSpan={7} style={{ padding: 20, textAlign: 'center', color: '#9CA3AF', fontSize: 13 }} data-testid="coupon-empty">No coupons yet</td></tr>
            ) : coupons.map((c, i) => (
              <tr key={c.id} data-testid={`coupon-row-${c.code}`} style={{ borderTop: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#F9FAFB' }}>
                <td style={{ padding: '9px 12px', fontSize: 13, fontWeight: 700, fontFamily: 'monospace', color: '#0D2E5A' }}>{c.code}</td>
                <td style={{ padding: '9px 12px', fontSize: 12, color: '#6B7280' }}>{c.discount_type === 'percentage' ? '%' : 'fixed'}</td>
