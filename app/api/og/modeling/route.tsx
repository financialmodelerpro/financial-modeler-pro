...
async function fetchAsBase64(url: string, label: string): Promise<string> {
  try {
    ...
    console.log(`[og/modeling] ${label} fetch: status=${res.status}`);
    if (!res.ok) return '';
    ...
    (
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, padding: '0 160px', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 48px', borderRadius: 48, background: 'rgba(27,79,138,0.18)', border: '2px solid rgba(27,79,138,0.45)', marginBottom: 64 }}>
            <span style={{ fontSize: 32, fontWeight: 700, color: '#93C5FD' }}>{badge}</span>
          </div>
          ...
          <div style={{ fontSize: 44, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6, maxWidth: 1400, textAlign: 'center' }}>
            {sub}
          </div>
        </div>

        <div style={{ padding: '40px 96px', borderTop: '2px solid rgba(255,255,255,0.08)', display: 'flex', justifyContent: 'center' }}>
          <span style={{ fontSize: 32, color: 'rgba(255,255,255,0.3)' }}>app.financialmodelerpro.com</span>
        </div>
      </div>
    ),
