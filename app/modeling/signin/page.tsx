...
function ModelingSignInInner() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/auth/register', {
      ...
      body: JSON.stringify({ name, email, password, phone: phoneCode + phoneLocal, city, country, captchaToken }),
    });
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex' }}>
                      <select value={phoneCode} onChange={e => setPhoneCode(e.target.value)}
                        style={{ padding: '10px 6px', fontSize: 13, border: '1.5px solid #D1D5DB', borderRadius: '7px 0 0 7px', borderRight: 'none', background: '#FFFBEB', cursor: 'pointer', fontFamily: "'Inter', sans-serif", outline: 'none', flexShrink: 0 }}
                        onFocus={e => { e.currentTarget.style.borderColor = BLUE; }}
                        onBlur={e => { e.currentTarget.style.borderColor = '#D1D5DB'; }}>
                        {COUNTRY_CODES.map(c => (
                      ...
                      <input type="tel" required value={phoneLocal} onChange={e => setPhoneLocal(e.target.value)}
