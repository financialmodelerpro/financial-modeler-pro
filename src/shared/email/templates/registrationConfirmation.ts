/*
 * html-safe: registrationId
 *
 * Generated by this codebase, alphanumeric.
 */
...
export async function registrationConfirmationTemplate({ name, registrationId, courseName }: RegistrationConfirmationData) {
  ...
  const subject = hasCourse
    ? `Welcome to ${escapeHtml(courseName)} - Your Registration is Confirmed`
    : 'Welcome to Financial Modeler Pro - Your Registration is Confirmed';

  const welcomeLine = hasCourse
    ? `Welcome to <strong>Financial Modeler Pro Training</strong>! You are now enrolled in <strong>${escapeHtml(courseName)}</strong>. Save your Registration ID - you will need it to sign in.`
  ...
  const textCourseLine = hasCourse
    ? `You are now enrolled in ${escapeHtml(courseName)}.`
