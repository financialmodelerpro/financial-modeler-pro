...
export type IcNarrativeFailure =
  ...
  /** The AI call itself failed. The credit IS spent (see the note below). */
  | { stage: 'ai'; reason: string; message: string; status: number; retryable: boolean }
  /** The call succeeded but returned nothing usable. */
  | { stage: 'empty'; reason: 'empty_response'; message: string; status: 502 };
...
export async function generateIcNarrative(input: GenerateIcNarrativeInput): Promise<IcNarrativeResult> {
  ...
  if (!result.ok) {
    // The credit is spent. That is the deliberate direction chosen in metering.ts:
    // consuming after a successful call would let concurrent requests all pass
    // the check first, which is the failure mode that actually costs money.
  ...
  if (!shaped.text.trim()) {
    return { ok: false, stage: 'empty', reason: 'empty_response', status: 502, message: 'The model returned an empty draft.' };
  }
