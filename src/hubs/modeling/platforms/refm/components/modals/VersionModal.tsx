...
export default function VersionModal({
  ...
  // ACTIVITY (step 6) and COMMENTS (step 7): the keyed-state fetch discipline
  // lives in useCollabData.ts, SHARED with the Module 10 screen, so the
  // "not recorded" vs "nothing happened" semantics cannot drift between the
  // two doors. Loaded lazily when the tab is opened, re-read each opening.
  const changesData = useProjectChanges(projectId, open && tab === 'activity');
  const commentsData = useProjectComments(projectId, open && tab === 'comments');
