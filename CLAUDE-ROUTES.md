...
└── platforms/
    ...
    # components/modules/Module4FixedAssets.tsx (NEW M4 Pass 1 UI 2026-05-19, commit 87f0075): Module 4 first surface. Consumes computeAllFixedAssetResults (from fixed-assets-resolvers.ts shipped in commit 1b5e9b9). Strategy-first wrapper: outer PhaseSection per strategy (Hospitality / Operations + Retail / Lease), nested PhaseDivider per phase, collapsible AssetSection per asset. Per-asset roll-forward PeriodTable: Opening NBV (last col aggregation) + Additions total + Land (sub-row, non-depreciable) + Depreciable basis (sub-row) + (-) Depreciation + Closing NBV (last col, grand total) + Accumulated Depreciation (last col). Closing project rollup PhaseSection (__project__) with the same shape for project totals. No engine changes; pure read.
