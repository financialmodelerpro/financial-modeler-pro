...
└── platforms/
    ...
    # lib/opex-resolvers.ts (NEW M3 Pass 2 2026-05-18, commit 8884d1b): bridge between Zustand store + M2 revenue snapshot + pure opex engine. computeAllOpexResults(state, revenueSnap) walks every non-Sell asset, builds per-asset OpexRevenueContext (room/F&B/other/total/lease per period sourced from byHospitalityAsset or byLeaseAsset), resolves [opsStartIdx, opsEndIdx] from phase + per-asset operationsStartYearOverride, derives total keys / leasable sqm from M1 sub-units, and calls computeAssetOpex. Also runs the HQ engine against project total revenue. Returns ProjectOpexSnapshot { byAsset, projectTotals (5 buckets + GOP + NOI), hq, totalOpexPerPeriodInclHQ }. Default-seeds via defaultHospitalityOpexLines / defaultLeaseOpexLines / defaultHQOpexLines when asset/project has no opex config.
    # components/modules/Module3Opex.tsx (NEW M3 Pass 2 2026-05-18, commit 8884d1b): Inputs tab. Phase-grouped collapsible asset cards mirroring Module 2 layout. Per-asset OpexLineTable with mode-aware value cell (PercentageInput when mode is pct_*, AccountingNumberInput otherwise), inflation toggle + rate input, on/off checkbox, remove + add-line buttons. HQ section at the top spans the project axis. Each strategy gets its own allowed-modes + allowed-categories palette (Hospitality: 17 categories + 7 modes; Lease: 6 categories + 4 modes; HQ: 4 categories + 2 modes). "Seed default lines" button on empty assets.
