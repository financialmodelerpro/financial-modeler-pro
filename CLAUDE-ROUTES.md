...
└── platforms/
    ...
    │   │   ├── modules/ (Module1AreaProgram, Module1Costs, Module1Financing, Module1Hierarchy, Module1Timeline, Module2Revenue [NEW M2 Pass 1 2026-05-16 commit 3e9c453, asset-list grouped by strategy with per-asset Configure Revenue button - Sell opens Module2SellModal, others stub])   # Module1Area DISSOLVED M1.12 2026-05-06 (Land tab eliminated, parcel CRUD lifted into ProjectWizard Step 2 + Build Program Land Parcels block)
...
└── verify-revenue-rebuild.ts                # NEW M2 Pass 2 2026-05-16 (commit 8ebaa80), EXTENDED Pass 4 (commit 83de2ac). Module 2 Residential Sell engine verifier. Fixture A (synthetic Point-in-Time + no escrow, 14 assertions including cohort catchup arithmetic at every period). Fixture B (MAAD T2 with 1BR 47,800 sqm @ 33,456 + 2BR 36,497.1 @ 33,505, over-time profile [0.30,0.30,0.30,0.10], Wafi 4% release Y6 - pre-sales total 2,539,827 SAR'000 reconciles within 0.0014% on every cell). Fixture C (Pass 4: 2-cohort 50/50 split of B MUST sum cell-for-cell identical, 28 assertions all delta=0.00) + Fixture C2 (cross-cohort velocity overflow flagged by reconciler). 76 pass / 0 fail / 76 total.
```
