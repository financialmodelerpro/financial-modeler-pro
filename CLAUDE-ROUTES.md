...
└── platforms/
    ...
    │   │   # lib/cases/applyOverrides.ts (NEW Cases Phase 1 2026-06-03): pure case-merge engine. applyOverrides(base, overrides) = inverse of diffSnapshots (sets each path on a clone; missing entities skipped = value-only). buildOverrides(base, edited) derives the map via diffSnapshots. getByPath, seedCases (Management/Downside/Upside), baseCaseId, normaliseCases. verify-cases.ts 19/19.
    ...
    # lib/excel/{styles,buildModelWorkbook,liveModel}.ts (FULL LIVE REBUILD 2026-06-11; was Phase-1 foundation 2026-06-04): FORMULA-DRIVEN .xlsx via ExcelJS, 14 sheets, fully formula-linked so editing any input flows to the IRR. `buildModelWorkbook(opts)` -> Workbook + `generateModelWorkbookBuffer` -> ArrayBuffer (ExportModal "Excel" option). KEY pattern: every calc cell = `fcell(formula, result)` (live Excel formula + cached value from `liveModel.ts`, a pure TS twin computing the SAME arithmetic so the workbook opens correct, recalculates on edit, and is self-consistent). Sheets: Cover/Index, Assumptions (inputs), Timeline, Land & Area, Capex (these 5 kept from the prior build, unchanged), then REBUILT LIVE: Revenue (base x per-period profile%), Cost of Sales (revenue x cost ratio), Opex (revenue x ratio + HQ), Financing (the ENGINE: depreciation, interest = rate x opening debt, tax, deficit-funded debt/equity drawdown + surplus cash-sweep recurrence, a clean forward recurrence, NO circularity), P&L / Cash Flow (link-and-present), Balance Sheet (rolls forward, balances by construction, verified diff=0), Returns (NOI + terminal value + FCFF/FCFE streams + live IRR()/NPV()/MOIC, IRR seeded with the twin's value as the convergence guess), Checks. SIMPLIFIED vs the platform engine (escrow / milestone-AR / IDC-capitalisation / multi-method funding are simplified); magnitudes follow the same drivers. FAST colors (navy-pale input / black formula / green link), defined names (ProjectStartYear/TaxRate/DebtPct/EquityPct/DiscountRate/ExitMultiple/ExitYearOffset...). Verifiers: `scripts/verify-excel-export.ts` (67, structure + liveness + reconciliation) + `scripts/verify-excel-recalc.ts` (7, HyperFormula evaluates the formula graph: BS balances, FCFF/FCFE streams recompute cell-for-cell, NPV matches the twin, an input edit moves the output). See memory project_excel_full_rebuild_2026-06-11. lib/pdf/generateProjectPdf.ts ALSO has `generateSummaryPdf` (concise executive-summary report: cover + exec summary + key inputs + headline P&L/CF/BS + returns cards).
