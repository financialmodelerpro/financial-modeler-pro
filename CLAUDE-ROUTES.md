...
└── verify-psync.ts                          # NEW Phase P-Sync 2026-05-07, Platform & Module Admin Sync verifier (canonical green). 5 sections: 18 SQL marker checks against p_sync_platform_modules.sql + 4 route file checks + smoke against /api/platforms/refm/modules + 20 lib helper / type checks against src/shared/cms/platform-modules.ts + 26 source-file markers across admin pages, sidebar hook, marketing pages, CmsAdminNav + em-dash sweep across 12 new files + Playwright presence/run gate (tests/e2e/psync-flow.spec.ts). 70 pass / 0 fail / 3 skip without dev server.
```
