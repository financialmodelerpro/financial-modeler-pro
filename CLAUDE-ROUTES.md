...
└── platforms/
    ...
    # lib/pdf/generateProjectPdf.ts (NEW 2026-06-02, commit `5db031d`): full-project PDF export on pdf-lib (already in package.json; NOT react-pdf). Pure `generateProjectPdf({state, projectName, versionLabel, dateLabel, selectedModuleKeys}): Promise<Uint8Array>`, reads computeFinancialsSnapshot + computeReturnsSnapshot (no new calc). Cover page (name/version/date/KPIs) + one navy-headed (#1B4F8A) section per selected module: M1 overview/funding/phases/tranches/cash-waterfall, M2 revenue+CoS, M3 opex, M4 P&L/CF/BS, M5 returns summary + sponsor streams. Landscape A4; wide period tables split across pages with label+Total cols + header repeating per column-chunk (PERIODS_PER_PAGE≈10) + on vertical overflow; footer = page/project/version. Table model `PdfTable{kind:'period'|'grid', columns, rows}` + `periodRow(label, values, 'sum'|'last'|'none', emphasis)`. Sections are registry-driven from MODULES (modules-config) with an EXPORTABLE set {module1..5}, so future modules auto-appear. Verifier `scripts/verify-pdf-export.ts` (7/7). NEXT: Excel export same structure.
    # components/modals/ExportModal.tsx (REWORKED 2026-06-02, commit `5db031d`): two-step flow: format grid (PDF Full Report live) → registry-driven module checkboxes (default all enabled+exportable) → Generate PDF → Blob download. pdf-lib imported lazily. Reads useModule1Store.getState(); gets projectName + versionLabel props from RealEstatePlatform (existing topbar Export button opens it).
...
├── verify-pdf-export.ts                     # NEW 2026-06-02 (commit 5db031d). Smoke test for lib/pdf/generateProjectPdf: builds a hotel+debt+dividends fixture, renders the PDF headless, asserts bytes>0, valid PDF parse, page count >= cover + selected modules, subset < full, empty selection = cover-only. 7/7.
