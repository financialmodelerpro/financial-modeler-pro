...
└── platforms/
    ...
    # core/calculations/opex/ (NEW M3 Pass 1 2026-05-18, commit 8884d1b): Module 3 Operating Expense engine. 5 files: types.ts (OpexLine + OpexLineMode union of 9 modes [fixed_baseline / pct_of_room_rev / pct_of_fb_rev / pct_of_other_rev / pct_of_total_rev / pct_of_lease_rev / per_room_year / per_sqm_year / pct_of_gop] + OpexLineCategory union of 19 categories + AssetOpexInputs / AssetOpexResult / HQOpexInputs / HQOpexResult + OpexRevenueContext); assetOpex.ts (computeAssetOpex two-pass evaluator: Pass A resolves every non-GOP line mode against the asset's revenue context, Pass B aggregates Direct + Indirect buckets, derives GOP = TR - Direct - Indirect, then fills pct_of_gop lines with the resolved GOP; returns bucket aggregates direct/indirect/mgmt/other + GOP + GOP margin + NOI); hqOpex.ts (project-wide HQ engine restricted to fixed_baseline + pct_of_total_rev modes); defaults.ts (KPMG SC7-mirroring seeds: 15 hospitality lines [rooms/F&B/other dept directs, G&A/IT/S&M/POM/Energy/EOSB indirects, mgmt base/tech/incentive, replacement reserve, rent & insurance, property tax], 5 lease lines [property mgmt, CAM, utilities, insurance, property tax], 4 HQ lines [payroll, office, professional, other]); index.ts (public exports). Verifier scripts/verify-opex.ts 26/26.
    # lib/opex-resolvers.ts (NEW M3 Pass 2 2026-05-18, commit 8884d1b; UPDATED commit faf16c8 2026-05-18): bridge between Zustand store + M2 revenue snapshot + pure opex engine. computeAllOpexResults(state, revenueSnap) walks every Operate or Lease asset (Sell + Manage parents and pure Sell are skipped — only the companion / pure hospitality / lease side carries opex), builds per-asset OpexRevenueContext (room/F&B/other/total/lease per period sourced from byHospitalityAsset or byLeaseAsset), resolves [opsStartIdx, opsEndIdx] from phase + per-asset operationsStartYearOverride, derives total keys / leasable sqm from M1 sub-units, and calls computeAssetOpex. Also runs the HQ engine against project total revenue. Returns ProjectOpexSnapshot { byAsset, projectTotals (5 buckets + GOP + NOI), hq, totalOpexPerPeriodInclHQ }. Default-seeds via defaultHospitalityOpexLines / defaultLeaseOpexLines / defaultHQOpexLines when asset/project has no opex config.
    ...
    # components/modules/Module3Opex.tsx (NEW M3 Pass 2 2026-05-18, commit 8884d1b; EXTENDED commits faf16c8 + 062c168 2026-05-18): Inputs tab. Phase-grouped collapsible asset cards mirroring Module 2 layout. Per-asset OpexLineTable with mode-aware value cell (PercentageInput when mode is pct_*, AccountingNumberInput otherwise), 3-button segmented Inflation control (Off / Flat / Yearly always visible; Yearly auto-expands a per-year % input strip on the row below; Yearly disabled with tooltip on HQ since project-wide has no asset ops window), on/off checkbox, remove + add-line buttons. HQ section at the top spans the project axis. Each strategy gets its own allowed-modes + allowed-categories palette (Hospitality: 17 categories + 7 modes; Lease: 6 categories + 4 modes; HQ: 4 categories + 2 modes). "Seed default lines" button on empty assets. **Asset filter** (faf16c8): only Hospitality (strategy='Operate', incl. Sell + Manage companions) and Retail/Lease show; Sell + Manage parents + pure Sell are excluded. **Apply to all Hospitality / Retail** button per asset card (faf16c8) clones current asset's lines onto every other same-strategy asset with fresh per-asset line ids. opsYearsForAsset() helper computes the per-asset operations window from phase + operationsStartYearOverride for the Yearly editor.
    # components/modules/Module3OpexOutput.tsx (NEW M3 Pass 2 2026-05-18, commit 8884d1b): Output tab (read-only). Per-asset PeriodTable shows Drivers (Total Revenue) -> Direct costs -> GOP + GOP margin -> Indirect costs -> Mgmt fee + reserve -> Other fixed charges -> Bottom line (Total Opex + NOI). Phase-grouped. HQ + Project Total tables at the bottom. PeriodTable supports per-row rowFmt + aggregation ('sum' / 'last' / 'avg').
    # lib/state/module1-types.ts (M3 Pass 2 2026-05-18, commit 8884d1b): added optional Asset.opex.lines[] (per-asset) and Project.hqOpex.lines[] (project-wide). Both shapes mirror the engine's OpexLine. Optional so legacy snapshots load without throwing.
    ...
    # components/RealEstatePlatform.tsx (M3 Pass 2 2026-05-18, commit 8884d1b): new exported m3Tabs (2 tabs: Inputs + Opex Output). New 'module3' branch in the render switch mirrors module2's tab bar + no-project guard. Imports Module3Opex + Module3OpexOutput.
...
└── verify-opex.ts                           # NEW Module 3 Opex engine verifier (Pass 1 baseline 2026-05-18, commit 8884d1b). 26 / 26 across 5 series: A (Hospitality directs / indirects / GOP / mgmt incentive % of GOP / NOI / GOP margin, 12 tests), B (Lease % of lease rev / per-sqm CAM with inflation / property tax / insurance / total opex / no stray per_room lines, 6 tests), C (indexation YoY compound 3% ratio, 1 test), D (disabled line stays zero + drops the indirect bucket, 2 tests), E (HQ engine fixed_baseline + pct_of_total_rev + total inside ops window, 5 tests). Fixtures use synthetic 100-key hotel + 5000-sqm retail asset against the default seed line lists.
```
