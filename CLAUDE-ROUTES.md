...
└── platforms/
    ...
    # lib/pdf/generateProjectPdf.ts (NEW 2026-06-02 `5db031d`; FULL REPORT + Inter font 2026-06-03): full-project PDF on pdf-lib (NOT react-pdf). Pure `generateProjectPdf({state, projectName, versionLabel, versionComment?, dateLabel, selectedModuleKeys, moduleSections?}): Promise<Uint8Array>`, reads computeFinancialsSnapshot + computeReturnsSnapshot (no new calc). Portrait Cover (name/version/comment/date/location/currency/scale/horizon + 6 KPIs + FMP branding) → portrait Project Description (mandatory: about + phases + assets-by-phase) → one landscape section per selected module split into Inputs/Outputs/Schedules sub-bands in tab order (per-asset detail from snapshot maps + cash/recognition vintage matrices). Each builder returns `ModuleContent{inputs,outputs,schedules}`; caller picks parts via `moduleSections[key].{inputs,outputs,schedules}` (missing=>included). Mixed orientation via `newPage({landscape,headerTitle})`+`contentW(ctx)`; grids `align:'kv'|'data'`; `Fmt` bundle keeps area/count/percent/ratio off the currency scale. **Font: embeds Inter (the platform UI font) via @pdf-lib/fontkit + base64 modules `lib/pdf/fonts/interRegular.ts`+`interBold.ts`, FULL embed `subset:false` (subset:true crashed Acrobat with "cannot extract the embedded font Inter-Bold-NNNN"); full Unicode (Δ, − render natively, no WinAnsi/ASCII fallback).** Wide period tables split per column-chunk (PERIODS_PER_PAGE≈10) + on overflow; footer = page/project/version. Registry-driven from MODULES with EXPORTABLE {module1..5}. Verifier `scripts/verify-pdf-export.ts` (16/16). NEXT: Excel export same structure.
