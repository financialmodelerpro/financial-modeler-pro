...
└── platforms/
    ...
    # lib/excel/{styles,buildModelWorkbook}.ts (NEW 2026-06-04, Excel model export, Phase 1 foundation): FORMULA-DRIVEN .xlsx via ExcelJS. `buildModelWorkbook(opts)` -> Workbook + `generateModelWorkbookBuffer` -> ArrayBuffer. KEY: every calc cell = `fcell(formula, result)` (live formula + cached snapshot value = dynamic AND reconcilable). FAST colors (blue input / black formula / green link), inputs only on Assumptions, defined names (ProjectStartYear/TaxRate/DebtPct/DiscountRate). Phase 1 sheets: Cover/Index, Assumptions, formula Timeline, Checks/legend. Verifier `scripts/verify-excel-export.ts` (15/15). ExportModal 3rd option "Excel Model (beta)". Phases 2-9 (calc + statement + returns sheets) per memory project_excel_export_plan_2026-06-04. lib/pdf/generateProjectPdf.ts ALSO gained `generateSummaryPdf` (concise executive-summary report: cover + exec summary + key inputs + headline P&L/CF/BS + returns cards).
