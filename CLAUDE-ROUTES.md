...
└── platforms/
    ...
    │   │   ├── modules/_shared/tableStyles.ts   # NEW M2.0 Pass 12 2026-05-13 (commits 2618e27 / 02dbd0b / 6ad624c / 88a55c0), single source of truth for results-table styling across REFM Module 1. Exports: TABLE_HEADER_BLUE + TABLE_HEADER_TEXT + ROW_SUBTOTAL_FILL color constants; ROW_DATA / ROW_ASSET_HEADING / ROW_SUBTOTAL / ROW_GRAND_TOTAL cell-level style objects (each as { name, num } pair for label + numeric cells); CELL_HEADER (navy fill, white uppercase bold, centered horizontally + vertically) and TABLE_TITLE (display block, fontSize 13, fontWeight 700, marginBottom var(--sp-1)). Internal CELL_BASE sets verticalAlign:'middle' + borderTop/Bottom:'none' so the project-wide `td { border-bottom: 1px solid var(--color-border) }` in app/globals.css line 319 stops painting a striped grid through data rows. **EXTENDED M2.0 Pass 13 2026-05-13 (commit 1c89859):** added `COLUMN_WIDTHS = { total: 110, period: 75, labelMin: 200 }` + `tableMinWidth(periodCount)` helper + `whiteSpace:'nowrap'` on every numeric/header cell token. Every results table renders `<table style={{ width:'100%', tableLayout:'fixed', minWidth: tableMinWidth(axis.count) }}>` with a shared `<colgroup>` so stacked tables share column widths and re-flow uniformly when granularity changes the period count. Consumed by Module1ProjectPhases / Module1Assets / Module1Costs / Module1Financing.
    │   │   ├── modules/_shared/periodAxis.ts    # NEW M2.0 Pass 13 2026-05-13 (commit 1c89859), universal period-axis builder. Exports `buildResultsPeriodAxis({ startIso, granularity, numAnnualPeriods, cropAnnualOffset? })` returning `{ priorLabel, activeLabels, labels, count }` where `labels` is `[priorLabel, ...activeLabels]`. Prior label = calendar period immediately before activeLabels[0] (Dec YY at annual / Q4 YY at quarterly / Dec YY at monthly). Wraps the existing `generatePeriodLabels` core helper. Consumed by Module1Costs Tab 3 Results (Tables 1-4) + Module1Financing Tab 4 Inputs Capex Breakdown + Inputs Summary + Schedules. Pure layout helper, no engine output change.
    │   │   ├── modules/_shared/GranularityRadioBar.tsx   # NEW M2.0 Pass 13 2026-05-13 (commit 3038e34), shared Annual/Quarterly/Monthly pill bar that writes to `project.outputGranularity`. Props: granularity + onChange + radioName (required so multiple bars on the same page stay independent) + optional label/helper/dataTestid. Consumed by Module1Costs Tab 3 Results and Module1Financing Tab 4 Inputs Capex Breakdown. Tab 4 Schedules keeps its inline bar because the granularity radios there are bundled with facility-filter pills.
