...
└── platforms/
    ...
    # components/modals/ExportModal.tsx (REWORKED 2026-06-02 `5db031d`; per-part toggles 2026-06-03): step 1 format grid (PDF Full Report live) → step 2 registry-driven module checkboxes, EACH with an Inputs/Outputs/Schedules toggle row (default all on), PLUS a number-scale radio (Thousands / Millions, default Millions, 2026-06-03); builds `moduleSections` + `displayScale` + Generate PDF → Blob download. pdf-lib imported lazily. Reads useModule1Store.getState(); projectName + versionLabel from RealEstatePlatform.
