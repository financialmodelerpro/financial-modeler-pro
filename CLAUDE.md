...
**Last updated: 2026-05-17. Module 1 (REFM) LOCKED at M2.0 Pass 58 (2026-05-16). Module 2 (Revenue) WIP, Residential Sell flow live through Pass 7 (4-tab sub-tab structure under Module 2 with Inputs / Revenue / Cost of Sales / Schedules surfaces). M2 Pass 7 (commit pending, 2026-05-17): four sidebar sub-tabs under Module 2 mirror Module 1's m1Tabs pattern. Tab 1 keeps the existing Pass 5/6 phase-wise asset-card inputs. Tab 2 renders project-axis tables for pre-sales revenue, post-sales revenue, recognition (P&L), and cash collected per asset + project totals. Tab 3 adds Cost of Sales recognised in step with revenue (matching principle): CoS[i] = totalCapex × recognition[i] / totalRecognition; cumulative CoS reaches total capex at end of recognition. Tab 4 surfaces working-capital schedules: Accounts Receivable (cum rec - cum cash, floored at 0), Unearned Revenue (cum cash - cum rec, floored at 0; mirrors AR), Escrow Balance, Net Cash to Developer. Engine additions: src/core/calculations/revenue/{accountsReceivable, unearnedRevenue, costOfSales}.ts plus revenue-resolvers.ts in the REFM lib for project-wide aggregation. Verifier scripts/verify-m2-pass7.ts 12/12 pass; existing 76/76 baseline unchanged. MAAD Excel at repo root is the verification benchmark, NOT a behavioural spec, every MAAD-specific behaviour stays configurable. Engine storage stays project-axis-indexed across all passes; only the UI window changes. See CLAUDE-REFM.md.
