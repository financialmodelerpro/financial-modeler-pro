...
**Last updated: 2026-05-16. Module 1 (REFM) LOCKED at M2.0 Pass 58 earlier today. Module 2 (Revenue) WIP - Residential Sell flow shipping in 4 passes (1: shell + asset-list grouped by strategy; 2: pure engine baseline matching MAAD T2 verifier 46/46; 3: per-asset form modal with velocity grid + cash/recognition profiles + escrow + indexation + live preview; 4: multi-cohort tabs with per-cohort velocity + price + profile overrides, cross-cohort velocity-sum identity, verifier 76/76 including Fixture C 2-cohort split). MAAD Excel at repo root is the verification benchmark, NOT a behavioural spec - every MAAD-specific behaviour is configurable so the engine serves the wider real estate market. New status `'wip'` added to ModuleStatus union with amber WIP pill. Prior Module 1 fine-tuning today (Passes 56-58): Pre-Capex split into Land + Building/Infra for future depreciation; sub-unit Count column directly editable in Units mode; reconciler double-count fix for existing facility raised inside project axis. **Module 1 LOCKED. Module 2 in build, Phase 1 Residential active.** See CLAUDE-REFM.md.**
