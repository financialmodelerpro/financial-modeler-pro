...
**Last updated: 2026-04-27.** This session: (**Multi-phase admin cleanup — dead Announcements stub removed, Pricing Features + Module Access tabs deleted, White-Label feature removed, Branding slimmed to colors-only, Permissions / User Overrides / Plans system deleted (REFM premium features locked to false pre-launch), migration 144 drops 5 dead tables. Earlier in the day: Newsletter system rebuild — batch send, recipient log, segments, scheduling, Resend webhook, click tracking, signup opt-in, editable templates; Marketing Studio logo render fix (satori width:auto bug); LinkedIn banner gradient fix; Communications Hub consolidation; Marketing Studio flexibility upgrade.**)
