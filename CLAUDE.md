...
- **Module 3 (Operating Expenses)** — WIP. Pass 4 complete: per-asset line-item engine (Hospitality + Lease) + HQ corporate overheads + Inputs UI + restructured Output. KPMG SC7 hierarchy expressed as configurable line modes. Each line carries `rateMode: 'single' | 'yoy'` so the user can either enter one value (Single, compounds via Asset Inflation) or supply a per-period rate array (YoY, bypasses inflation). Output tab is now category-wise tables per asset (Revenue Breakdown → Direct → Indirect → Management → Reserves for Hospitality; Revenue → Property Operating → Recoveries (memo) → Other Charges for Retail) plus a category-headed project rollup at the bottom. No GOP / NOI rows on Output — those compose in M4. Verifier `scripts/verify-opex.ts` 38/38 green.
