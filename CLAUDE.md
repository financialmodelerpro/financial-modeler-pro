...
**Last updated: 2026-05-14. Current REFM phase: M2.0 Passes 37-48 (16 commits). Highlights: every "existing operation" input is now single-source-of-truth on Tab 4 -> 1b. Existing Operations card (per-asset Pre-Capex/Debt/Equity, per-phase opening BS, Existing Retained Earnings); Tab 1 Historical Baseline + duplicate Current Debt Outstanding fully removed; engine reads per-asset values via existing.ts (Pass 38). Tab 3 Costs tile bar = Land/Hard/Soft/Total Capex Excl. Land. Tab 4 Schedules + KPIs gate every existing-only block on `isActiveExisting()`. Tab 4 inputs all match navy-pale form style (Pass 42). migrationsApplied marker stops banner-on-every-reload (Pass 43). Dashboard redesigned (hero / 6 KPI tiles / module deck / phase share-bar / reconciliation chips / version history); legacy Overview sidebar entry removed. Modeling Dashboard + REFM marketing page DB-driven; admin Module Manager consolidated, Add/Remove platforms wired. Migrations 150 + 151 applied. See CLAUDE-REFM.md.**
