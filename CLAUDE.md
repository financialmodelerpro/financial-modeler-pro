...
**Last updated: 2026-05-14. Current REFM phase: M2.0 Passes 37-47 (Tab 4 polish + Module 1 audit + Dashboard redesign + admin DB-driven), 14 commits. Highlights: Finance Cost (Existing) KPI split when active existing facility; Tab 1 Historical Baseline trimmed to opening BS items only (engine now reads per-asset Pre-Capex/Equity); Current Debt Outstanding moved entirely to Tab 4 -> Existing Facility -> Opening Balance (single source of truth); Costs tile bar dropped Operating, added Total Capex Excl. Land; isActiveExisting() filter gates every existing-only block on Schedules (Debt Movement / Combined Debt Service / Finance Cost / IDC Summary); 13 unstyled inputs on Tab 4 now match navy-pale form style; migrationsApplied marker so banners fire once not every reload (Pass 7 Costs notice); Tab 4 Existing Operations Summary card (per-asset tile cross-check + per-phase opening BS editor + new Existing Retained Earnings field); Dashboard fully redesigned (hero, 6 KPI tiles, 4-card module deck, phase summary with share-bar, reconciliation chip strip, version history); legacy Overview entry removed from sidebar, aliased to Dashboard; Modeling Dashboard + REFM marketing page now DB-driven (Cycle Status / Add / Remove platforms via /admin/platform-modules); admin Module Manager consolidated into one page. Migrations 150 (p_sync_platform_modules) + 151 (extend modules marketing) applied. See CLAUDE-REFM.md.**
