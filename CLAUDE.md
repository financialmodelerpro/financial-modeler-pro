...
**Last updated: 2026-05-14 (EoD). Current REFM phase: M2.0 Passes 37-55 (23 commits in one session). Highlights: Existing Operations card collapsed INTO each Existing Facility's form on Tab 4 (per-phase BS + per-asset baseline + totals render only when phase is Operational, panel sits BELOW rate + repayment rows so users finish facility identity first); per-asset Existing Debt is the SOLE input - facility Opening Balance is read-only and auto-synced via useEffect (Pass 54), removing the mismatch tile and any duplicate-entry friction; YoY % schedule editor now spans full operations horizon when Repayment Periods is unset (Pass 55, single-2026 bug). Dashboard Existing Operations tile shows Pre-Capex headline (= Debt + Equity sublabel) instead of double-counting all three (Pass 51). Tab 1 Historical Baseline + duplicate Current Debt Outstanding fully removed; engine reads per-asset values via existing.ts (Pass 38). Tab 3 Costs tile bar = Land/Hard/Soft/Total Capex Excl. Land. Tab 4 Schedules + KPIs gate every existing-only block on isActiveExisting(). migrationsApplied marker stops banner-on-every-reload (Pass 43). Dashboard redesigned + Rules-of-Hooks crash fixed (47/47b). Modeling Dashboard + REFM marketing page DB-driven; admin Module Manager consolidated, Add/Remove platforms wired. Migrations 150 + 151 applied. **Status: closed for the day; tomorrow = fine-tuning + lock Module 1.** See CLAUDE-REFM.md.**
