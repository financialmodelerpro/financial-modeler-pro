...
- **Module 2 (Revenue + Cost of Sales + Schedules + Escrow)** — LOCKED at Pass 9M. M2 Pass 9L moves Sell+Manage companion (Operate side) from Residential to Hospitality bucket on Revenue Inputs. M2 Pass 9L-Fix: setters rebuild byPhase from full legacy axis + hydration migration backfills existing snapshots. M2 Pass 9M: sticky asset quick-nav strip on all per-asset M2 surfaces. M2 Pass 9N: cohort tail-catchup (positions past axis settle at N-1) closes the AR-stuck-on-axis-end bug. Verifiers `verify-revenue-rebuild` 133/133 + `verify-escrow` 46/46 + `verify-cost-of-sales-v2` 24/24 green.
- **Module 3 (Operating Expenses)** — LOCKED at Pass 5d. Opex Inputs grouped by strategy bucket (Hospitality / Retail-Lease) with companion link chip. Verifiers `verify-opex` 38/38 + `verify-opex-ap` 24/24 green.
- **Module 4 (Financial Statements)** — WIP at Pass 2N-Fix. M4 composer + BS surfaces rebuilt across the off-by-one debt class: `FacilityResult.outstanding[t]` is closing balance at end of year t (length N); engine arrays are length N indexed by project year (no prior column). 11 sites changed from `.slice(1, 1+N)` to `.slice(0, N)`; new `FacilityResult.openingBalance` exposes the pre-axis balance for the BS prior column. BS Share Capital fix: includes pre-axis equity opening (priorEquity + cumulative new draws, no double-count from engine's lump). A1 + L2 in BS Schedules now literal per-asset mirrors of M2 Revenue Output. CF financing block consolidated by origin (Existing / New buckets) with phase tag dropped + Pre-Capex row + Finance Cost capitalised memo. M2 CoS Capex by stage now includes IDC line. M4 P&L + CF strategy buckets show subtotal inline on dropdown header (no separate Total row). Total column +25% width. Excel-style trace-to-source scaffolding (`M4Row.trace` + ⤴ icon). Verifiers `verify-fixed-assets` 82/82 + `verify-idc-depreciation` 22/22 + `verify-asset-cost-allocation` 14/14 + `verify-m4-bs-reconciliation` 59/59 green. Total verifier sections: **486/486 green** across 11 scripts.
