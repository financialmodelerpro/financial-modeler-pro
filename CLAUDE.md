...
**Last updated: 2026-05-13. Current REFM phase: M2.0 Pass 12 (Universal formatting standards + Tab 4 Financing deep refactor), 15 commits. Themes: shared `_shared/tableStyles.ts` token suite (ROW_*, CELL_HEADER, TABLE_TITLE, ROW_SUBTOTAL fill + borders), blur-formatting `AccountingNumberInput` + new `PercentageInput`, Tab 3 phase button row + default-collapse, Tab 4 Asset Filter removal + Method 2 per-line engine wire + Existing Operations origin flag + YoY% editor + Mixed IDC + Deferred Payment editor + Capital Structure collapsible + method-based land-inkind lookup. Layered on M2.0 Pass 11 + Tab 3 Critical Regressions Round 2 + Tab 2 Pass 1-3 + M2.0 Pass 10. See CLAUDE-REFM.md.**
