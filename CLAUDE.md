...
**Last updated: 2026-04-21** (launch-readiness session — **Inline certificate issuance** now fires the moment a student passes their final exam via fire-and-forget from `/api/training/submit-assessment` calling new engine helper `issueCertificateForStudent(email, courseCode)`; old daily `/api/cron/certificates` route deleted along with its `vercel.json` schedule; **`student_certificates.email_sent_at`** timestamp stamped after issuance email resolves (migration 124) with partial index `idx_student_certificates_email_unsent` for constant-time resend lookups; **Admin safety-net panel** at `/admin/training-hub/certificates` — "🛟 Eligible but not issued" list with per-row ⚡ Issue Now + bulk Issue All Pending, plus an Email column with Unsent / Sent pill and ✉ Resend button; three new admin routes `GET /api/admin/certificates/pending`, `POST /api/admin/certificates/issue-pending`, `POST /api/admin/certificates/resend-email`; **Legacy Certificate Generation admin tile removed** — the `/admin/certificates` Automatic Generation toggle + ⚡ Generate Now button + their `/api/admin/certificates/settings` + `/api/admin/certificates/generate` routes + the now-unused `processPendingCertificates()` engine function all deleted; **WhatsApp Group Link** — admin-editable `training_settings.whatsapp_group_url` (migration 123) surfaces as a green Join WhatsApp Group CTA in the Training Hub dashboard sidebar when set; public read via `GET /api/training/community-links` with server-side URL-shape re-validation; empty value hides the button entirely (no broken state); **Context-aware live-session achievement card** — `/api/training/achievement-image` now accepts `has_assessment` + `duration` params; with assessment = green score circle + PASSED pill, without assessment = teal duration circle + ATTENDED pill, with duration chip on both variants; 3SFM/BVM unchanged because legacy call sites omit the new params. First pass missed `SessionCard.tsx` which is the actual component for recorded live sessions via the `RecordedLiveSessionRow` adapter — follow-up commit threaded `hasAssessment` + `durationMinutes` through the adapter so the attended variant renders for watched recordings; prior pre-launch work still applies — Resume/Continue Watching via `playerVars.start`, Video-swap auto-detection, Mark Complete in final 20s, Onboarding Tour (migration 120), Auto-launch cron wired-but-disabled (migration 118), per-registration session reminders (migration 122), Coming-Soon bypass list (migration 121))
