...
**Last updated: 2026-04-24.** This session: (**Admin auth consolidation + redirect-loop nuclear fix + stale `proxy.ts` deletion (real root cause) + Vercel edge cache headers + Platform Analytics + live-session UX rebuild + communications overhaul + Training Hub registration cleanup + Certificate Designer hub + SEO redirect/canonical fixes + Marketing Studio rebuild from scratch + Marketing Studio multi-instructor / photo upload / zone-based drag-resize + Vercel build fix (sharp out of client bundle) + sitemap /home deny-list**)
