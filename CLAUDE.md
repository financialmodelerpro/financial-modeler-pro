...
**Last updated: 2026-04-18** (session end — Marketing Studio Phase 2 (multi-platform FMP presets + Quick Fill auto-populate + multi-caption + ZIP export), Phase 1.5 + background library + aspect-ratio lock + FMP YouTube preset (migration 102), drag-and-drop canvas editor, react-rnd, element-based designs, multi-asset brand kit, migration 101, universal CmsField rendering path, Tiptap RichTextarea upgrade, array-item VF, retake + timer persistence, attempts counter server-authoritative, founder_profile table dropped, /about page removed, universal share utility, Calendly inline embed on /book-a-meeting, migrations 097-102)
...
- **Presets**: `src/lib/marketing/presets.ts` — element-based starting points. `PRESETS` array: **FMP YouTube Thumbnail** (1280×720, signature brand design), **FMP LinkedIn Post** (1200×627, headline + founder mini-card + series tag), **FMP Instagram Post** (1080×1080, centered headline + gold divider + founder portrait), YouTube Thumbnail (generic), LinkedIn Post (generic), Instagram Post (generic), Instagram Story 1080×1920, Blank Custom. Each exports `buildPreset(brandKit) → { background, elements }`. FMP presets use meaningful element-id prefixes (`title-`, `subtitle-`, `insight-`, `headline-`, `session-`, `title2-`, `tag-`, `series-`) so Quick Fill + ZIP export can remap text content across formats. `FMP_EXPORT_PRESET_IDS` drives the ZIP export target list.
