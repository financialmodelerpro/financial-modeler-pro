...
**Last updated: 2026-05-17 (EoD). Module 1 (REFM) LOCKED at M2.0 Pass 58 (2026-05-16). Module 2 (Revenue) WIP, Residential Sell flow live through Pass 7k. Pass 7g removed the project-wide Sell template; every Sell asset owns its own cash / recognition / indexation. Pass 7h restructured the Revenue Output tab around a single per-asset narrative (1 SQM Sold pre/post/total+cum%, 2 Revenue pre/post/total, 3 Recognition vintage matrix, 4 Cash vintage matrix, 5 AR roll-forward, 6 Unearned roll-forward) with a formula caption on every table. Pass 7i reordered the Inputs tab: Recognition then Cash both on full rows, every calc grid carries a Total column, Indexation Start Year + Step builder restored. Pass 7j applies MAAD-style YoY rounding (whole units or whole sqm) BEFORE revenue is derived. Pass 7k swapped the AR / Unearned engines to MAAD's BS Build roll-forward floored (Closing = MAX(0, Opening +/- flows)), exposing openingPerPeriod + changePerPeriod for CF wiring. Verifier scripts/verify-revenue-rebuild.ts 25 / 25 green; Pass 7h block structure already applies to every Sell asset in every phase (no per-asset gating). MAAD Excel at repo root is the verification benchmark, NOT a behavioural spec, every MAAD-specific behaviour stays configurable. Engine storage stays project-axis-indexed across all passes; only the UI window changes. See CLAUDE-REFM.md.
