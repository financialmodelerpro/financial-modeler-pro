...
**Last updated: 2026-04-29.** This session: (**Model-submission guidance copy hardening on top of the watch-enforcement removal. Watch enforcement system removed; Mark Complete simplified back to a video-ended trigger that surfaces 20 seconds before end. Two commits. Commit `f583c70` deleted the global watch-enforcement system (5 files removed: `app/api/training/watch-enforcement/route.ts`, `app/api/admin/watch-enforcement-stats/route.ts`, `src/hubs/training/components/WatchProgressBar.tsx`, `src/hubs/training/lib/watch/watchEnforcementCheck.ts`, `src/hubs/training/lib/watch/watchThresholdVerifier.ts`) plus the entire Watch Enforcement admin card on `/admin/training-settings` (global toggle, threshold slider, per-session bypass table, search + filters + sort, bulk row actions, summary stats, `SummaryStat` + `TypeBadge` helpers, `LiveSessionRow` / `SessionRow` / `SessionKind` types, `saveEnforcement` / `saveGlobalEnforcement` / `saveThreshold` / `toggleBypass` helpers). Server-side: dropped the 403-on-threshold block + `manual_override` checks from `/api/training/certification-watch` + `/api/training/live-sessions/[id]/watched`, dropped `verifyWatchThresholdMet` from `certificateEngine.issueCertificateForPending`, dropped `loadWatchEnforcement` + `watchThresholdMet` + `watchDetails` from `certificateEligibility` (and the consumer in `/admin/training-hub/certificates`). Client-side: stripped enforcement state, `watchPct` / `thresholdMet` / `bypassActive` / `canMarkComplete` derivation, `manualOverrideAvailable`, `watchHint`, `WatchProgressBar` mount, fetch to `/api/training/watch-enforcement`, and `handleManualComplete` callback from both 3SFM/BVM cert watch page + live-session watch page. Replaced the gate with `videoEnded && !markedComplete ? handleMarkComplete : undefined`. Dropped dead `watchPercentage` / `watchThreshold` props from `SessionCard`, `CourseContent`, `RecordedLiveSessionRow`, `LiveSessionCardLarge`, and the dashboard's `pctMap` / threshold state + watch-enforcement fetch. Dropped manual override block + ghost watch hint from `CourseTopBar`; dropped `onManualComplete` / `watchHint` / `belowVideoContent` props from `CoursePlayerLayout`. Dropped 20s near-end report-throttle bypass from `YouTubePlayer`. `LiveSessionCardLarge` assessment button now always renders (no watch threshold gate); achievement card eligibility for non-assessment recordings now keys off `status='completed'` instead of `pct >= threshold`. Settings keys `watch_enforcement_enabled` / `watch_enforcement_threshold` / `watch_enforcement_bypass_*` stay in `training_settings` (no migration churn) but no code reads them anymore. The interval-merging tracker stays running (analytics only): `certification_watch_history` / `session_watch_history` continue to receive `watch_seconds` / `total_seconds` / `last_position` / `watch_intervals` writes for admin Watch Progress + Platform Analytics. `videoTimer.ts` + `detectVideoChange.ts` + `watchTracker.ts` retained. Skip-to-end is permitted by design: certificate credibility now comes from the model-submission gate (migration 148), not watch percentage. Net commit `f583c70`: -1548 lines across 21 files. Commit `f790fa9` then changed the tick fallback inside `YouTubePlayer` from `currentTime >= duration - 1` to `currentTime >= duration - 20` so Mark Complete surfaces with 20 seconds of playback remaining (student watches the wrap-up + clicks immediately instead of waiting for the very last second + the next tick). Detection chain: tick at `c >= d - 20` (primary), `PlayerState.ENDED` (final fallback), PAUSED-at-`d - 1` (corner case for scrub-to-last-second + pause before next tick). Single-fire guarded by `endedFired`. Both watch pages share the `onEnded` prop so the change applies uniformly. Type-check + production build clean for both commits; lint baseline improved 251 -> 248 problems (-3 warnings, errors unchanged). **No new files, no new packages, no schema changes, no new API routes** in this session — the work was a removal + simplification. Three follow-up commits then tightened the model-submission copy: `6df1970` rewrote the per-course `DEFAULT_GUIDANCE` constant in `src/hubs/training/components/dashboard/ModelSubmissionCard.tsx` so the baked-in fallback reads "Our experts team will review it within 5 business days" instead of "An admin will review it" (admin override at `training_settings.model_submission_guidance_<course>` still wins when populated; constant is a pure code-level last resort and never written to the DB so existing customizations survive deploys). `9a81f71` updated CLAUDE.md / CLAUDE-ROUTES.md / CLAUDE-FEATURES.md / CLAUDE-DB.md / CLAUDE-TODO.md / PROJECT_HANDOFF.md to reflect the watch-enforcement removal + d-20 unlock change. `01904dc` then deleted a stale hardcoded "Explainer" `<div>` block at `ModelSubmissionCard.tsx:423-428` that pre-dated the Phase F.2 `<GuidancePanel>` and was rendering a duplicate "Build your own financial model for X..." paragraph immediately above it — the student dashboard was showing the guidance text twice, once with the old "admin" wording and once with the corrected "experts team" wording. Same commit replaced the three remaining occurrences of "An admin will review" across the codebase: soft-launch banner copy in `ModelSubmissionCard.tsx:269` ("An admin will review it on an effort-based pass/reject basis" → "Our experts team will review it..."), upload success toast in `ModelSubmissionCard.tsx:231` ("Submitted. An admin will review your model..." → "Submitted. Our experts team will review your model..."), and the Final Exam gate copy under the panel in `CourseContent.tsx:446` ("submit the financial model you have built using the panel above. An admin will review it within 5 business days" → "...Our experts team will review it..."). Grep for `An admin will review` returns zero matches. Net diff for `01904dc`: 2 files, +5/-12 lines. No new files, no new packages, no schema changes, no new API routes for any of the three follow-up commits — pure copy + dead-code removal. Type-check + production build clean across all three.**) Prior 2026-04-29 model-submission-gate work kept verbatim below.

## 2026-04-29 session summary — Certificate credibility upgrade (migration 148, 6 phases + 4 hardening sub-phases, dormant by default)

(Earlier same-day initiative, kept verbatim from before the watch-enforcement removal block was added above.)

**Certificate credibility upgrade — model-submission gate, 2 phases of schema-and-gate work then 4 phases of hardening, all dormant by default until admin flips. Migration 148 lands the foundation: new `model_submissions` table (email + course_code + attempt_number with a partial unique index on `LOWER(email)`, status enum pending_review/approved/rejected, partial index on the pending hot path), new private `model-submissions` storage bucket (no public SELECT - admin reads via a service-role proxy), seven seeded `training_settings` keys including the per-course `model_submission_required_<course>='false'` gate flips and the soft-launch `model_submission_announcement_only='true'` banner toggle. Phase A (commit c2b02e4): schema + `getModelSubmissionStatus()` reader. Phase B (commit c42239d): server-side gate inside `certificateEngine.issueCertificateForPending` + `/api/training/submit-assessment` final-exam path with fail-open semantics so an unflipped gate never blocks issuance. Phase C (commit 70d45fd): student `<ModelSubmissionCard>` 6-state machine on the dashboard (soft-launch banner / locked / pending review / approved / rejected with attempts left / exhausted) + `POST /api/training/model-submission` (FormData upload, attempts cap, one-pending guard, file-type + size validation, MIME normalization). Phase D (commit bddb5e2): admin queue `/admin/training-hub/model-submissions` + review modal + `POST /api/admin/model-submissions/[id]/review` with audit log + `GET /api/admin/model-submissions/[id]/file` private-bucket proxy + 2 email templates (Approved + Rejected). Phase E.1 (commit 38a0bec): dashboard hoists the status fetch (parallel `?courseCode=3SFM` + `=BVM`), `<SessionCard>` grew an optional `lockReason` prop, Final-exam SessionCard swaps to a "Submit your model" lock when the gate is on, `/training/assessment/[*_Final]` client-side gate redirects to `/training/dashboard?gate=model-submission`. Phase E.2 (commit 9cd4f66): Model Submission Gate admin card on `/admin/training-settings` with audit-logged toggles (`POST /api/admin/training-settings/model-submission-gate` writes `admin_audit_log.action='model_submission_gate_change'` with before/after values + confirm dialog before every flip). Phase F.1 (commit 059aa40): admin alert email fires fire-and-forget via `next/server` `after()` whenever a new submission lands. Two new settings (`model_submission_admin_notify_enabled` / `..._email`) + admin UI inside the Gate card; empty recipient OR disabled toggle = OFF. Phase F.2 (commit 68ba5de): per-course guidance text + optional sample template URL. Two settings keys per course; baked default guidance per course used when admin has not populated the setting; URL scheme restricted to `http(s)` at the read boundary. Phase F.3 (commit a67b4bf): daily 08:00 UTC cron `/api/cron/model-submission-stale` emails an admin digest of `pending_review` rows older than `model_submission_stale_threshold_days` (default 2). Reuses F.1 enable + recipient settings. Phase F.4 (commit 06074b9): `scripts/model_submission_notice_broadcast.ts` CLI for the existing-student notice email broadcast. Per-scope idempotency stamp (`model_submission_notice_broadcast_<scope>_at`) plus `--scope`/`--dry-run`/`--limit`/`--force` flags + audit row per run. Cutover order: notice broadcast -> wait `notice_days` -> flip per-course `required_<course>` ON in admin -> optionally flip `announcement_only` OFF. Force-issue (`/admin/training-hub/certificates`) keeps bypassing the gate as the admin escape hatch. **Manual action required**: apply migration 148 via Supabase dashboard (idempotent). After deployment, populate the F.1 admin email recipient + per-course F.2 guidance, then run `npx tsx --env-file=.env.local scripts/model_submission_notice_broadcast.ts --scope all --dry-run` to preview, then drop the `--dry-run` to broadcast.**) Prior 2026-04-28 folder-restructure work kept verbatim below.

## 2026-04-28 session summary — Folder restructure for portability (8 phases, complete)

**Folder restructure for portability — 8 phases, all green on Vercel. The flat `src/lib/` + `src/components/` layout was reorganized into `src/core/` (pure business logic) + `src/shared/` (cross-hub primitives) + `src/hubs/{main,training,modeling}/` (hub-scoped code, including `modeling/platforms/refm/` for individual modeling platforms) + `src/features/` (Marketing Studio) + `src/integrations/` (third-party SDK adapters). Phase 2.1 scaffolded target folders. Phases 2.2-2.5 moved ~220 files via `git mv` so history is preserved (commits `Restructure 2.2` through `Restructure 2.5`). Phase 2.6 cleaned up the original 5 cross-hub violations from PLATFORM_INVENTORY: relocated the share family (`ShareModal.tsx`, `share.ts`, `shareTemplates.ts`, `useShareTemplate.ts`) to `src/shared/share/`, dependency-inverted `comingSoonGuard` into a pure `shouldGateComingSoon` primitive at `src/shared/comingSoon/guard.ts` plus per-hub adapters (`@training/lib/ensureNotComingSoon`, `@modeling/lib/ensureNotComingSoon`), and decoupled `COURSES` from shared shareTemplates by extracting the COURSES-aware resolver to `@training/lib/share/resolveCourseName.ts`. Phase 2.7 added 8 path aliases (`@core`, `@shared`, `@training`, `@modeling`, `@platforms`, `@main`, `@features`, `@integrations`) and `eslint-plugin-boundaries` enforcement at lint time — CI now blocks any new cross-hub regression. Phase 2.8 was documentation only: scoping table updated to new paths, ARCHITECTURE.md added with the rationale + alias guide + how-to-add-platform recipe, follow-ups documented (NextAuth `authorize()` dependency inversion, J.10 admin middleware centralization, CLAUDE.md size, optional narrow `@/*` alias). Net source-tree state: 0 cross-hub violations on the original 5; one TODO-tracked deferred suppression in `nextauth.ts` for the planned NextAuth refactor. All routes still serve, no behavior changes, no migrations needed.** Prior 2026-04-28 watch-tracking + admin-cleanup work kept verbatim below.

## 2026-04-28 session summary — Watch tracking rebuild (migrations 146 + 147)

**Goal**: kill the multi-session resume bug that left students stuck under threshold despite watching to completion (Fakhri reported watching 3SFM_S2 fully but the tracker read 47%). Pre-launch diagnosis identified 12 failure modes; this session shipped a four-phase rebuild plus a surgical recovery sweep.

- **Phase 2 (commit `c9a20e4`, mig 146)** — Smoking-gun fix. Persist `watch_intervals JSONB` on both `certification_watch_history` and `session_watch_history`. Pre-146 the schema only persisted scalar `watch_seconds`; the in-memory tracker rebuilt seconds from intervals but threw them away on unmount. On a return visit the tracker re-seeded with `baseline = persisted_watch_seconds` and `intervals = []`, so `max(baseline, sumNew + open)` froze at the largest single contiguous run. Now the tracker hydrates from the JSONB on mount, POSTs a snapshot of merged intervals every progress tick, and the server unions incoming + existing then re-derives `watch_seconds = sumIntervals(merged)` with a wall-clock rate limit on the new portion. Five additional tracker fixes: (a) `onPlay` closes any pre-existing open interval before opening a new one (covers PLAYING -> BUFFERING -> PLAYING glitches), (b) cross-session interval union via `initialIntervals` prop, (c) BUFFERING treated as a soft pause, (d) tracker stored in `useRef` so external prop updates re-seed without remount, (e) `WatchProgressPayload` with a `force` flag set on real close events so consumers bypass POST throttle and the final partial interval lands in the DB.

- **Phase 3 (commit `13cb260`, mig 147)** — Manual override path. New columns `completed_via TEXT NULL` (`'threshold'` / `'manual'` / `'admin_override'`) and `video_load_at TIMESTAMPTZ NULL` (server-stamped on first POST per row). UI: CourseTopBar gains a checkbox-gated "I confirm I have watched this video" + Mark Complete button when watch% is in the [50, threshold) band. Server enforces `pct >= 50` AND `wall-clock elapsed >= total_seconds * 0.8` before honouring; a 403 with diagnostic info bounces tampered submits. `video_load_at` is server-set so clock-tampered clients cannot advance it; re-anchored on detected video swap.

- **Phase 4 (commit `e2dd9a4`)** — Visibility. `WatchProgressBar` re-enabled (was a no-op return null pre-Phase 4 because the pre-146 tracker had race conditions that made the displayed % unreliable). Color-coded fill + dashed threshold marker + bypass-aware copy + three label buckets keyed off the [50, threshold) band so students know the override path exists when they hit it. New admin endpoint `POST /api/admin/sessions/[tabKey]/force-complete-for-student` mirrors `reset-watch-progress`'s prefix routing (LIVE_ vs cert), flips status='completed' + completed_via='admin_override', clamps `watch_percentage` to actual coverage, awards +50 points on live-session rows that hadn't received them, writes to `admin_audit_log` with action='watch_force_complete'. Idempotent. Admin students panel's Progress modal gains a Watch Progress table below the Sessions table with per-row Force Unlock buttons. The endpoint reuses the existing `/api/admin/training-hub/student-progress` extended to return `{ progress, watch }`.

- **Phase 5 (commit `670fb51`)** — Surgical recovery for 4 stuck students identified by `scripts/diagnose_stuck_watch.ts`. Sweep ran via `scripts/phase5_recovery.ts` (service-role, mirrors the endpoint logic; the HTTP endpoint requires NextAuth admin cookies which are awkward to thread from a CLI - same precedent as migration 140 / 141 service-role scripts). Targets unblocked, all confirmed by `scripts/phase5_verify.ts`: `muhammadtayyabmadni07@gmail.com` (3SFM_S1, 100% -> completed), `yusra.tufail@yahoo.com` (3SFM_S1, 93%), `daniyal1012@yahoo.com` (3SFM_S1, 76%), `fakhrizanul@gmail.com` aka FMP-2026-0035 / Fakhri (3SFM_S2, 47%). Every row stamped `completed_via='admin_override'` + audit row with `via='phase5_recovery_script'`. **Notable finding**: 3 of 4 stuck students were on `3SFM_S1` specifically. Could be coincidence (S1 has the highest enrollment) or session-specific edge case worth investigating if more students surface stuck on the same session post-fix. No batch email per the simplified Phase 5 plan; their dashboards will simply work on next login. Fakhri gets a personal reply from Ahmad (draft staged at gitignored `TEMP_FAKHRI_EMAIL.md`).

- **Three unlock paths going forward**: `threshold` (auto at >=70%, default `training_settings.watch_enforcement_threshold`), `manual` (student override at >=50% + server-side elapsed-time check), `admin_override` (admin force-unlock from the panel). The 70% threshold itself is unchanged. `WatchProgressBar` exposes the actual % to students because the manual override needs them to know they're in the [50, threshold) band; the literal threshold percentage is still hidden in the CourseTopBar ghost hint copy.

**Manual action required**: Migrations 146 + 147 must be applied via Supabase dashboard. Both idempotent (`ADD COLUMN IF NOT EXISTS`). Pre-migration rows have `[]` / null and the legacy scalar acts as a floor so students mid-course don't lose existing credit.

## 2026-04-28 session summary — Branding merge + Pricing simplification (migration 145)

## 2026-04-28 session summary — Branding merge + Pricing simplification (migration 145)

(Earlier same-day initiative, kept verbatim from before the watch-tracking rebuild block was added above.)

**Goal**: continue trimming admin surfaces after the 2026-04-27 cleanup. Three cleanups:

- **Part A (commit `ab5db30`)** — Branding merged into Header Settings. After 2026-04-27's Phase 4 slimmed `/admin/branding` to two color fields, the dedicated page was a thin wrapper around two inputs. Brand Colors section moved into `/admin/header-settings` as the first card, wired to the same `/api/branding` GET + PATCH endpoints as before. `saveAll()` now fires the cms_content writes plus a single `/api/branding` PATCH in parallel. `/admin/branding/page.tsx` reduced to a 5-line server `redirect('/admin/header-settings')` so existing bookmarks keep working. `BrandingThemeApplier` untouched — same `branding_config` table, same `--color-primary` / `--color-secondary` injection. Sidebar entry "Branding" removed; Header Settings gains `matchPaths: ['/admin/branding']` so the rail stays highlighted on stale links. Net -349 / +102.

- **Part B-2 (commit `50e22fa`)** — Pricing Page Content tab removed. Diagnosis surfaced a real bug: the tab wrote to `cms_content` (section='pricing_page') but Page Builder writes to `page_sections` (slug='pricing'). The public `/pricing` page read from `cms_content`, meaning Page Builder edits for the pricing slug were dead writes. Migration 046 had already seeded `page_sections` with the right shape (hero / cards / faq / cta), so the fix was to repoint `/pricing` at `getAllPageSections('pricing')` and delete the now-orphan tab. Hero badge / title / subtitle now resolve from `pricing.hero` section content; FAQ items resolve from `pricing.faq` section's `items[]` (with per-item `visible !== false` filter). Tab type narrowed to `'plans' | 'platform'`. ~100 lines net.

- **Part B-1 (commit `777e1bf`)** — Plans tab removed + migration 145 drops `pricing_plans`. The generic Free/Starter/Professional/Enterprise plan catalog was the original pricing model but was never wired into payment or feature gating — `platform_pricing` + `platform_features` + `plan_feature_access` (migs 076/077) is the canonical per-platform pricing model that actually drives the public pricing page. The home-page pricing-teaser plan-name pill row in `app/(portal)/page.tsx` was the only public consumer; replaced with a clean "View Full Pricing →" CTA-only block. With Plans gone and the Page Content tab gone too, only Platform Pricing remained — so `/admin/pricing/page.tsx` was rewritten with no tab bar at all (single-purpose surface). Files deleted: `app/api/admin/pricing/plans/route.ts`, the local `getPublicPlanNames()` helper in home page, and the orphan `getPublicPlanNames()` export in `src/lib/shared/cms.ts`. `Plan` / `UserOption` / `FormState` types + `BLANK_FORM` constant + `PlanCard` sub-component + `planFromRow` / `savePlan` / `deletePlan` / `duplicatePlan` / user-search effect all gone. Migration 145: `DROP TABLE IF EXISTS pricing_plans CASCADE`. Net -598 / +156.

**Manual action required**: Apply migration 145 via Supabase dashboard SQL editor. Idempotent (`IF EXISTS` + `CASCADE`) so re-runs are no-ops.

**Re-introducing premium gating in the future**: same posture as 2026-04-27 — when Modeling Hub paid tiers launch, build a focused new pricing/subscription system server-enforced from day one, shaped around the actual product surface that exists at that point.

## 2026-04-27 session summary — Admin cleanup (migration 144)

**Goal**: clean foundation before Modeling Hub expansion. Six dead admin surfaces had accumulated over earlier feature pivots — admin pages without users, API routes that wrote to DB tables nothing read, type unions for features no UI exposed. Each one was load-bearing in some past world but is now a maintenance tax with zero user value. Path A (aggressive removal) was chosen because Modeling Hub is still pre-launch with ~6 whitelisted testers — there's no production load to preserve compatibility with.

**Net total**: -3164 lines across 33 files, 11 admin pages/components/hooks/types deleted, 4 API routes deleted, migration 144 drops 5 underlying tables (`user_permissions`, `plan_permissions`, `features_registry`, `pricing_features`, `pricing_modules`).

- **Phase 1 (commit `fd0aabf`)** — Dead Announcements stub. `/admin/announcements` page + AnnouncementsManager.tsx (full CRUD UI) + `/api/admin/announcements` route all queried a non-existent `announcements` table. The whole feature was a stub from a planned-then-abandoned sitewide-banners flow. -325 lines, sidebar entry removed.
- **Phase 2 (commit `4a5abe3`)** — Pricing Features + Module Access tabs. The Plans + Page Content + Platform Pricing tabs of `/admin/pricing` are all live. The other two tabs wrote to `pricing_features` (per-plan checklist) and `pricing_modules` (per-plan module gate) — neither the pricing page nor the Modeling Hub Modules admin (`/admin/modules`, separate `modeling_modules` table) ever read them. Tab UI deleted (~316 lines), `/api/admin/pricing/modules/` route deleted (Features API stays, still used by Platform Pricing tab).
- **Phase 3 (commit `a000fbd`)** — White-Label feature. `/admin/whitelabel` + `useWhiteLabel` hook + `BrandingConfig.whiteLabel` field were admin-write-only with REFM Topbar as the lone consumer (per-client name/logo override for "Acme Capital" style branding). Topbar now reads platform name + logo directly from the branding store via `getPlatformLogo()`. The `pdf_whitelabel` export tier in REFM Export Modal is preserved (tier label only, gating now stubs to false). -390 lines, sidebar entry removed.
- **Phase 4 (commit `ee959ad`)** — Branding slim-down. `/admin/branding` had Portal Identity (platformName / portalTitle / portalSubtitle / portalDescription / footerText) + Logos (portalLogo* / platformLogo*) + Brand Colors sections. Header Settings + Page Builder are canonical for site content + page logos; the first two sections wrote into `BrandingConfig` fields that no live surface read (besides the dead `BrandingSettingsPanel.tsx` that was never imported). Slimmed to Brand Colors only. `getPortalLogo()` helper deleted. `BrandingThemeApplier` simplified to read `branding.primaryColor` directly. The orphan `BrandingSettingsPanel.tsx` deleted. -1054 lines.
- **Phase 5 (commit `d8405e5`)** — Permissions / User Overrides / Plan Configuration. The migration 006 trio (`features_registry`, `plan_permissions`, `user_permissions`) backed three sibling admin pages that all wrapped the same `<PermissionsManager>` component (486 lines) writing through the same APIs. The system WAS read by REFM at runtime via `useSubscription()` → in-memory cache, but no server-side enforcement existed — gating was advisory only. With paid tiers not yet live and Modeling Hub being rebuilt, Path A: rip out the entire stack. REFM premium features now lock to `false` until paid tiers launch (existing `<UpgradePrompt>` overlays + lock indicators continue to render correctly because the API surface is preserved — only the data source is stubbed). Files deleted: 3 admin pages, 2 API routes, `src/lib/shared/permissions.ts` (server resolver), `src/hooks/useSubscription.ts` (client cache), `src/components/admin/PermissionsManager.tsx`, `src/types/subscription.types.ts` (FeatureKey union, plan types). Inline-replaced: `ExportModal` / `UpgradePrompt` / `PlanBadge` keep small `'free' | 'professional' | 'enterprise'` union locally now that subscription.types.ts is gone. `core/branding.ts` lost the unused `USER_SUBSCRIPTION` stub + `hasAccess()` helper. `SystemHealth` lost its `/api/permissions` probe. -1169 lines, 2 sidebar entries removed (User Overrides + Permissions).
- **Phase 6 (commit `b8b6df9`)** — Migration 144 drops the 5 underlying tables. `DROP TABLE IF EXISTS … CASCADE` on each so dependent FKs/views/triggers go with them and re-runs are no-ops. **Apply manually via Supabase dashboard before deploy.**
- **Phase 7 (this commit)** — CLAUDE-DB.md / CLAUDE-FEATURES.md / CLAUDE-ROUTES.md updated to reflect the cleanup. Pricing / White-Label / Branding feature rows reclassified.

**Re-introducing premium gating in the future**: when Modeling Hub paid tiers launch, the right place to resurrect plan-based feature locking is a focused new feature spec — not by reverting these commits. The old system tried to be infinitely flexible (per-plan AND per-user overrides AND a feature catalog) and ended up being all admin and no enforcement. A new system can be smaller, server-enforced from day one, and shaped around the actual product surface that exists at that point.

## 2026-04-27 session summary — Newsletter rebuild (migration 143)

**Goal**: replace the sequential `resend.emails.send()` loop + hardcoded template fork with a production newsletter pipeline that's auditable, retryable, segmentable, scheduled, click-trackable, and DB-driven across both manual sends and event auto-notifications. **Migration 143** is required and must be applied manually via Supabase dashboard (no programmatic apply path was reachable from this session — DB pooler creds stale, no `exec_sql` RPC). The new code paths are forward-compatible: queries against the new tables return empty until 143 is seeded.

- **`newsletter_recipient_log` (new table)** — per-recipient row keyed by `(campaign_id, email)`. Status enum: pending / sent / failed / bounced / complained / opened / clicked. Stores `resend_message_id`, `error_message`, `sent_at`, `opened_at`, `clicked_at`. Mirrors the migration 138 pattern from announcements. Drives the Campaigns analytics modal + retry-failed flow.
- **`newsletter_templates` (new table)** — single source of truth for both manual Compose and auto-notify. `template_key` UNIQUE, `subject_template`, `body_html`, `event_type` (matched by auto-notify), `active`. Seeded with 6 default templates that mirror the previously-hardcoded `generateContent` (NewsletterTab) and `generateEmail` (autoNotify) functions. Edit at `/admin/communications-hub?tab=newsletter` → Templates sub-tab; both code paths re-render through the same engine so manual-vs-auto template drift is gone.
- **`scheduled_at` + `segment` on `newsletter_campaigns`** — status CHECK extended to include `'scheduled'` and `'cancelled'`. Schedule path: send route stores the campaign with `status='scheduled'`, cron picks it up.
- **`src/lib/newsletter/sender.ts`** — central send pipeline. Resolves segment → seeds `pending` recipient_log rows → chunks 100 per batch → fires `resend.batch.send([100])` (rate-limited to 1 per second by Resend) → updates each row with the returned message_id + status → 200ms stagger between batches → updates aggregate counts on the campaign. Used by manual send, scheduled cron, auto-notify, and retry-failed (the last passes an explicit `recipients` list).
- **`src/lib/newsletter/segments.ts`** — 7 segments: `all_active`, `active_30_days`, `passed_3sfm`, `passed_bvm`, `never_started`, `has_certificate`, `no_certificate`. Composes the active subscriber set with filter sets from `certificate_eligibility_raw`, `student_certificates`, `training_assessment_results`. `targetHub='all'` deduplicates by email so people subscribed to both hubs only get one copy.
- **`src/lib/newsletter/templates.ts`** — `getTemplate(key)`, `getTemplateByEvent(eventType)`, `interpolate(template, vars)`, `renderTemplate(tpl, vars)`, `renderForEvent(eventType, vars)`. Missing tokens render as empty string (no ugly `{title}` fragments).
- **`src/lib/newsletter/linkWrap.ts`** — every `<a href>` rewrites to `/api/newsletter/click?msg={msg}&campaign=X&url=encoded`; UTM params (`utm_source=newsletter, utm_medium=email, utm_campaign=<id>`) appended on internal `financialmodelerpro.com` hosts. Unsubscribe URLs + the click endpoint itself are skipped. The `{msg}` placeholder stays in the body until per-recipient injection at send time (the Resend webhook is the canonical click-tracking path; the redirector falls back to a campaign-only redirect when `msg=` is the unfilled placeholder).
- **`POST /api/admin/newsletter/send`** — accepts `{ subject, body, targetHub, segment, scheduledAt? }` OR `{ templateKey, templateVars, ... }`. Inserts the campaign row, then either fires `void sendCampaign(...)` (immediate) or stamps `status='scheduled'` (cron picks it up). All admin role-gated.
- **`POST /api/admin/newsletter/test-send`** — "Send to my inbox" button. Renders subject+body or template, sends one email to the admin's session email (or supplied `toEmail`) with `[TEST]` prefix. No log row, no batch, no segment query.
- **`GET / POST /api/admin/newsletter/templates` + `PATCH / DELETE /api/admin/newsletter/templates/[key]`** — CRUD for the templates table. Variable schema (per event type) returned alongside the list so the Compose tab can show which `{tokens}` are valid.
- **`GET /api/admin/newsletter/segments?segment=X&targetHub=Y`** — returns `{ count, segments[] }` for the live recipient count display in Compose.
- **`GET /api/admin/newsletter/campaigns/[id]`** — single campaign + `recipients[]` from the log + computed `totals` (sent/failed/bounced/complained/opened/clicked/pending) + `openRate` + `clickRate`. **`PATCH /api/admin/newsletter/campaigns/[id]`** with `{ action: 'cancel' }` flips scheduled → cancelled. **`DELETE`** drops the campaign (recipient log rows cascade).
- **`POST /api/admin/newsletter/campaigns/[id]/retry`** — re-sends to every `failed` or `bounced` row. Resolves emails back to active subscriber rows (skips ones who unsubscribed in the interim), passes them to `sendCampaign({ recipients: [...] })`, then recomputes `sent_count` / `failed_count` from the full log so totals reflect cumulative successes (sender.ts only counts the retry batch by itself).
- **`GET /api/cron/newsletter-scheduled`** — bearer-auth via `CRON_SECRET`. Polls `newsletter_campaigns WHERE status='scheduled' AND scheduled_at <= NOW()` (limit 20 per tick), calls `sendCampaign()` for each. `vercel.json` cron entry runs daily at 07:00 (Hobby tier limit; upgrade to Pro for finer cadence — pattern matches the existing `/api/cron/auto-launch-check` carve-out).
- **`POST /api/webhooks/resend`** — Svix signature verification using Node `crypto.createHmac('sha256', ...)` so we don't pull in the `svix` package. Verifies headers `svix-id` / `svix-timestamp` / `svix-signature` against `RESEND_WEBHOOK_SECRET` (`whsec_<base64>` format), rejects timestamps older than 5 minutes (replay protection), supports multiple signatures per header. Routes events: `email.delivered` → stamp `sent_at` if null; `email.opened` → `opened_at` + status='opened'; `email.clicked` → `clicked_at` + status='clicked'; `email.bounced` → status='bounced' + (on hard bounce) flip subscriber `status='bounced'`; `email.complained` → status='complained' + flip subscriber `status='unsubscribed'`. Lookups by `resend_message_id` (globally unique). Configure in Resend dashboard → Webhooks → endpoint URL `<MAIN_URL>/api/webhooks/resend` → toggle the 5 event types.
- **`GET /api/newsletter/click`** — public click-tracking redirector. Best-effort updates the recipient row by `resend_message_id`, then 302s to the decoded `url` param. Always 302s even on internal failure so a tracking blip never breaks the user's click. Rejects non-http(s) URLs.
- **`src/lib/newsletter/autoNotify.ts`** — refactored to call `renderForEvent(eventType, vars)` first; falls back to a hardcoded shell only when the templates table has no row for the event (first-run before migration 143 seeds, or after manual deletion). After rendering, hands off to `sendCampaign()` so the same batch + recipient log + retry path applies as manual sends. Net result: a published article fires the same template the admin sees in the Templates editor, and the recipients show up in the same Campaigns analytics modal as manual sends.
- **Signup opt-in checkboxes** — `app/training/register/RegisterForm.tsx` + `app/modeling/register/RegisterForm.tsx` both gain a checked-by-default GDPR-friendly checkbox. After successful registration, `void fetch('/api/newsletter/subscribe', { hubs: ['training'|'modeling'] })` fires-and-forget — never blocks the success UI, errors silenced.
- **`NewsletterTab.tsx` rebuild** — five sub-tabs (Subscribers / Compose / **Templates (new)** / Campaigns / Auto Notifications). Compose adds: template picker, segment dropdown with description, schedule datetime picker, live recipient count, "Send test to my inbox" button, separate Schedule Send vs Send Now CTA. Templates is a list + edit view with per-event-type variable chips, name + event_type + active toggle + subject + RichTextEditor body. Campaigns shows the existing list but each row opens a new analytics modal: 6 stat cards (sent/opened/clicked/failed/bounced/complained), open + click rates, per-recipient table (sticky header, status badges, sent/opened/clicked timestamps, error column), `Retry N Failed`, `Cancel scheduled send`, `Export recipients (CSV)`, `Delete campaign`. Subscribers + Auto Notifications kept as-is.

**New env var (must be added in Vercel before the webhook works)**: `RESEND_WEBHOOK_SECRET`. Without it the webhook returns 500. Cron job `CRON_SECRET` is reused (no new var).

**Net code**: +1880 / -200 lines across `src/lib/newsletter/`, `app/api/admin/newsletter/`, `app/api/cron/newsletter-scheduled/`, `app/api/webhooks/resend/`, `app/api/newsletter/click/`, both register forms, and the rebuilt `NewsletterTab.tsx`. Type-check clean.
...
> **See also:**
> - [ARCHITECTURE.md](ARCHITECTURE.md) — Three-tier folder layout rationale, alias guide, boundary rules, how-to recipes for adding a platform / hub (added 2026-04-29 with Phase 2.8)
...
> - [PROJECT_HANDOFF.md](PROJECT_HANDOFF.md) — Resume-from-cold session brief
> - [RESTRUCTURE_PLAN.md](RESTRUCTURE_PLAN.md) — 8-phase folder restructure plan (executed 2026-04-28 → 2026-04-29; complete)
...
| `RESEND_WEBHOOK_SECRET` | Resend webhook signing secret (`whsec_...` format). Powers Svix-style HMAC-SHA256 verification at `POST /api/webhooks/resend` for newsletter delivery + open + click + bounce + complaint events. Configure in Resend dashboard → Webhooks → endpoint URL `<MAIN_URL>/api/webhooks/resend` → events: `email.delivered`, `email.opened`, `email.clicked`, `email.bounced`, `email.complained`. Hard-bounce auto-flips subscriber `status='bounced'`; complaint auto-flips `status='unsubscribed'`. |
...
- **Dashboard**: fetches watch history, passes `completedWatchKeys` + `inProgressWatchKeys` to SessionCard (the `watchPctMap` + `watchThreshold` props were dropped 2026-04-29 with the watch-enforcement removal)
- **SessionCard**: "Take Assessment →" only when `isWatched=true`; StatusBadge shows "In Progress" amber badge

### Mark Complete Gate (post-2026-04-29)

Uniform across the live-session watch page and the 3SFM/BVM certification watch page:

```
markCompleteCallback = videoEnded && !markedComplete ? handleMarkComplete : undefined
```

- `videoEnded` flips to `true` when the YouTubePlayer fires `onEnded`. Three triggers, single-fire guarded by `endedFired` inside the player:
  1. **tick fallback**: `currentTime >= duration - 20` during normal playback (primary unlock signal — Mark Complete surfaces 20 seconds before the video ends so the student can watch the wrap-up + click immediately).
  2. **YT PlayerState.ENDED** (final fallback for videos where the tick missed, e.g. mounted past d-20 or end-screen cards skip the event).
  3. **PAUSED-at-tail**: PAUSED with `currentTime >= duration - 1` (corner case: scrubbed to last second + paused before next tick fires).
- Skip-to-end is permitted by design. Certificate credibility now comes from the model-submission gate (migration 148), not from gating watch percentage.
- Server-side `/watched` + `/certification-watch` no longer 403 on threshold; they accept any `status='completed'` POST. The interval-merging tracker still posts `watch_seconds` / `total_seconds` / `last_position` / `watch_intervals` for analytics fidelity (admin Watch Progress + Platform Analytics), but no UI consumes the merged seconds.

**Removed 2026-04-29** (commit `f583c70`): the global watch-enforcement system (5 files deleted, the entire admin card on `/admin/training-settings`, and the cert-issuance threshold gate). Settings keys `watch_enforcement_enabled` / `watch_enforcement_threshold` / `watch_enforcement_bypass_*` remain in `training_settings` but no code reads them. See the session header at the top of this file for the full list of removed surfaces.
...
### Watch Tracker (interval-merging, migration 146; analytics only post-2026-04-29)
- **Interval-merging tracker**: `src/hubs/training/lib/watch/watchTracker.ts` — records `[start, end]` intervals from PLAYING → PAUSED/ENDED transitions, merges overlaps on every commit. Seeking forward, replaying, or skipping cannot inflate the count. A `baselineWatchedSeconds` seed ensures a reload with a higher DB value never makes the live counter go backwards.
- **YouTubePlayer**: accepts `baselineWatchedSeconds` + `initialIntervals` + `onProgress(watchedSec, totalSec, pos, intervals, force)`. Polls getCurrentTime every 1s during PLAYING, reports roughly every 10s (plus on pause/end/unmount/BUFFERING with `force=true`). Seek detection: if `|pos - (lastPos + 1)| > 2s` we close the previous segment and open a new one. The `c >= d - 20` tick check fires `onEnded` so Mark Complete surfaces 20 seconds before the video ends.
- **Persistence**: every progress POST writes `watch_seconds`, `total_seconds`, `last_position`, `watch_percentage`, `watch_intervals` JSONB to `certification_watch_history` (3SFM/BVM) or `session_watch_history` (live sessions). Migration 146 added the JSONB column so cross-session watch progress accumulates instead of being capped at the largest single contiguous run.
- **Read consumers (post-removal)**: admin Watch Progress panel (`/admin/training-hub/students` Progress modal), Platform Analytics dashboard, video-swap auto-detection (`detectVideoChange.ts`), live-session per-quiz `require_watch_before_assessment` opt-in gate (per-assessment, separate feature). No student-facing UI consumes the percentages anymore.
- **Removed 2026-04-29**: `WatchProgressBar.tsx`, `watchEnforcementCheck.ts`, `watchThresholdVerifier.ts`, `/api/training/watch-enforcement`, `/api/admin/watch-enforcement-stats`, the entire Watch Enforcement admin card. Settings keys `watch_enforcement_enabled` / `watch_enforcement_threshold` / `watch_enforcement_bypass_*` remain in `training_settings` but are unread.
