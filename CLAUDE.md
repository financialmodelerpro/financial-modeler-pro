...
**Last updated: 2026-05-13. Current REFM phase: M2.0 Passes 14-18 (Tab 4 Final Redesign + Verification Fixes), 29 commits. Themes: annual-only basis until M5 FS; data-driven period axis (no hard cap); 22% label / equal-width-others column model; Funding Requirement table with all 3 methods + Selected row; per-facility Grace Interest Treatment dropdown; per-asset pre-capex with debt+equity validation chip on Tab 1; Land Funding collapsed to Debt% / Equity% pair (deferred + custom-split + dropdown removed); Capital Structure Overview removed (match chip moved to Funding Basis); funding methods renumbered to 1/2/3; engine routes Land Cash uniformly via parcel ratios across all 3 methods; Schedules drawdown now derives from `funding.debtEquitySplit.debt × facilitySharePct/100` per facility; YoY% editor uses repayment periods (no P1-P60 default); Capital Stack Summary block removed from Schedules; Methods 2 + 3 rows blank in Funding Requirement until M2 Revenue + M4 FS ship. Layered on M2.0 Pass 13. See CLAUDE-REFM.md.**
