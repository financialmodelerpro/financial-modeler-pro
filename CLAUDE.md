...
- **Module 4 (Financial Statements)** — WIP at Pass 2Z. Built out Pass 2O → 2Z across this session (IDC policy split, BS Equity split + RE Schedule, IDC integrated into Fixed Assets roll-forward, Funding Gap sub-tab, Cash Sweep, Dividend waterfall with EBITDA cap, consolidated master waterfall, Phase 1 i=0 lump rescue + per-parcel in-kind stamping for BS balance, existing-debt double-count guard, IDC table column alignment fix). Verifiers: 693/693 sections green across 11 scripts (+228 sections this session). See `CLAUDE-REFM.md` for the Pass 2O-2Z narrative.
