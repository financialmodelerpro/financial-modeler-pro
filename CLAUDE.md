...
**Last updated: 2026-05-18. Module 1 (REFM) LOCKED at M2.0 Pass 58. Module 2 (Revenue + CoS + Schedules) LOCKED at Pass 9g-O. Module 2 (Revenue) WIP→LOCK history below; Residential Sell flow lived through Pass 7z then Pass 8/9 added Hospitality, Lease, vintage matrices, inventory roll-forward, the financial-statement feed Schedules, PIT custom recognition year, rental pool enrollment toggle (Auto-link / Day 1 full), Costs→Capex tab rename, Phase Overlap input retirement. Module 3 (Financial Statements) is the next module — P&L / BS / CF compose from M2's per-asset line items. Pass 7g removed the project-wide Sell template (per-asset only). 7h restructured Revenue Output around a per-asset Block A-F narrative (SQM pre/post/total+cum% -> Revenue pre/post/total -> Recognition vintage matrix -> Cash vintage matrix -> AR -> Unearned). 7i reordered Inputs (Recognition above Cash, both full row, Total column on every calc grid, Indexation Start Year + Step builder). 7j applies reference-style YoY rounding (whole units or whole sqm) BEFORE revenue derivation. 7q (AR + UR final) switched both schedules to sale-value driven roll-forward: AR closing = Opening + Pre-Sales Sale Value - Cash Received; UR closing = Opening + Pre-Sales Sale Value - Revenue Recognised. Both stay >= 0 and settle to 0 by end of contract lifecycle. 7r added Cumulative % row to Cash + Recognition profile strips (matches the reference "Cumulative Payment Profile" line) and confirmed the reference model Sales During Operation treatment (Calc_Retail & Resi. rows 172-188 = area * sale price * indexation, recognised + collected SAME period) already matches our engine. **7s surfaces Sales During Operation in Blocks 3 + 4 of Revenue Output: 3a = Pre-Sales recognition vintage matrix + 3b = Pre/SDO/Total recognition summary per period; 4a = Pre-Sales cash vintage matrix + 4b = Pre/SDO/Total cash summary per period. Project Total restructured into strategy-grouped per-asset breakdown: Residential/Sell (nested Pre-Sales per asset + SDO per asset + strategy total), Hospitality/Operations + Retail/Lease + Sell+Manage (per-asset, zero placeholders until Pass 8/9/10 engines wire in). Same grouped structure applies across Revenue (Sales Value) / Recognition / Cash. Engine unchanged (post-sales recognition + cash already same-period since 7f); pure UI surfacing. PeriodTable gained `'section'` row kind (colspan label-only) + per-row `indent` so nested groups render cleanly. 7t fixed Recognition Over-Time percentages being wiped when toggling to Point-in-Time. 7u merged Sell + Manage parents into Residential / Sell and Sell + Manage companions into Hospitality / Operations (no standalone Sell + Manage group); Revenue (Sales Value) view drops the Pre/Post nesting (sale value is timing-agnostic) while Recognition + Cash retain it; dropped the redundant `(SAR)` suffix from every PeriodTable title (currency lives on the tab header). 7v dropped Single-Rate from the indexation UI (kept None / YoY Compound / Step); collapsed the per-sub-unit velocity grid to a single "All sub-units" lockstep row by default with an opt-in "Split per sub-unit" toggle (engine storage stays per-sub-unit; collapsed-mode writes propagate to every sub-unit). 7w extends the Sell-side treatment to Sell + Manage parents: same Inputs tab (velocity, indexation, cash profile, recognition profile), same Output narrative (Blocks 1-6), same CoS + Schedules. Engine, resolver, and all 4 Module 2 surfaces now allow `strategy === 'Sell' || strategy === 'Sell + Manage'`. Companion (operate side) remains Hospitality / Operations placeholder until Pass 10. 7x adds a sub-unit reference chip strip at the top of every Sell asset on Inputs + Output tabs: read-only chips showing per-sub-unit name + area + sale rate (per unit or per sqm) so users can verify M1 Tab 2 entries without leaving the Revenue surface. 7y makes Output Block 1 metric-aware: assets with uniform `metric='units'` sub-units (e.g. Residential Tower 01 priced per apartment / hotel keys) render Block 1 in units with the section title "Units Sold"; uniform `metric='area'` assets render in sqm; mixed assets fall back to sqm with an inline note. Engine now exposes `presalesUnitsPerPeriodPerSubUnit` + `postSalesUnitsPerPeriodPerSubUnit`; reconciliation column renamed "Cum % Sold". Block 2 formula text now reads "Pre-Sales {Units|SQM} x base rate" depending on metric. 7z pins the PIT-at-handover convention: handover = LAST construction year (= `phaseStart + cp - 1 - projectStart`), NOT first operations year. "At handover" pill now shows the resolved year inline ("At handover (2029)") + Block 3 caption spells out the rule. 5 new verifier tests (A2-1..A2-5) pin the convention. **Pass 8 (shipped 2026-05-18) ships Hospitality end-to-end: engine (`src/core/calculations/revenue/hospitality.ts` — keys × ADR × occupancy + F&B + Other with 3 modes each: % of rooms / per-guest / baseline+growth; ADR indexation; ops-window clamping), resolver (`asset.revenue.operate` + `resolveHospitalityConfig` + computeAllSellResults walks Operate + companions), Inputs UI (Hospitality AssetCard variant), Output narrative (2 blocks per asset + Project Total integration), AR via DSO (default 30 days) on Schedules tab. Pass 8e (2026-05-18) added a new `yoy_per_period` indexation method — per-year variable growth array, compounds cumulatively — exposed on ADR (Inputs tab strip), available for Sell price indexation + F&B/Other ancillary indexation; "Fixed Annual" mode renamed to "Baseline + Growth" to mirror a baseline + growth structure; a one-click "stabilization curve" preset fills the occupancy ramp with 40 → 60 → 65 → 67% (typical 5-star). 43 new verifier scenarios (22 hospitality engine + 7 DSO AR + 9 yoy_per_period + 5 PIT handover pinning). Verifier 75/75. Pass 8f (2026-05-18) UI polish: occupancy strip drops the cumulative % row (per-year rate, not a cohort) and switches Total column to Avg; "DSO" renamed to "Accounts Receivable Days"; Output Block 1 (Operations Capacity) restructured into one merged table with Drivers section above (Total Rooms / Days / Occupancy / ADR Indexation Factor / ADR) and Calculations section below (ARN / ORN / Guests). PeriodTable gained per-row `rowFmt` + `totalOverride` so the same table can mix integer + % + ×factor + currency cells. Engine exposes `adrIndexationFactorPerPeriod`. Pass 9a (2026-05-18) restructured the Residential Cost of Sales surface per the reference design: each per-asset CoS table now opens with a DRIVERS section (Capex from M1 broken down by stage — Land / Hard / Soft / Operating + Total; then Revenue Recognition % per period + Cumulative %), then a CALCULATIONS section (Revenue Recognised, CoS matched = capex × recognition %, Gross Margin, Cumulative CoS). Local PeriodTable gained `isSection` row support. Land is explicitly included in CoS per user's v1.16 design (differs from KPMG which excludes land). Capex now read via full `computeAssetCost` (returns AssetCostBreakdown by stage) instead of just the total scalar.** Verifier scripts/verify-revenue-rebuild.ts 32 / 32 green. the reference Excel at repo root is the verification benchmark, NOT a behavioural spec, every reference-specific behaviour stays configurable. Engine storage stays project-axis-indexed; only the UI window changes. See CLAUDE-REFM.md.

**Module 2 LOCKED 2026-05-18 at Pass 9g-O. Verifier 133/133 green.** Engine + UI + verifier complete for Residential Sell, Hospitality (pure Operate + Sell+Manage companions), Retail/Lease. Schedules tab is a raw line-item financial-statement feed (Revenue / CoS / Gross Margin / Inventory / AR / Unearned / Cash / Capex per asset, grouped by strategy) — no derived totals (Direct CF, Indirect CF, NWC) here; those compose in M3.

**Next step: Module 3 (Financial Statements).** Compose full P&L (Revenue - CoS - opex - D&A = NI), BS (NWC + D&A schedule + retained earnings), CF (Direct + Indirect) from the M2 schedule inputs.
