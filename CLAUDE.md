...
**Last updated: 2026-05-13. Current REFM phase: M2.0 Passes 14-20 (Tab 4 Final Redesign + Schedules Rebuild + Equity Fix), 33 commits. Highlights: annual-only basis until M5 FS; data-driven period axis with operation-end cap on Schedules; off-by-one cropping (`totals[1..]`, `col = i - 1`) unified across Inputs + Schedules; Funding Requirement table with all 3 methods + Selected row (Methods 2/3 rows blank until M2 Revenue + M4 FS ship); Land Funding = Debt% / Equity% pair (deferred + custom-split + Capital Structure Overview removed); funding methods renumbered 1/2/3; Land Cash routed uniformly via parcel ratios across all 3 methods; per-facility Schedules drawdown = `funding.debtEquitySplit.debt × facilitySharePct/100` via `precomputedDrawSchedule`; legacy `tranche.drawdownMethod` switch removed from engine (schema stays @deprecated); Schedules rebuilt with 5 tables (Debt Movement, Combined Debt Service, Finance Cost, IDC Summary, Equity Movement) + filter pill bar; Grace Interest Treatment reshuffled to 4 options (capitalize default, raise_via_funding, raise_as_debt, pay_from_ocf) with Pass 20 migration; equity cash is additive to in-kind (Dec 26 zero-equity bug fixed). See CLAUDE-REFM.md.**
