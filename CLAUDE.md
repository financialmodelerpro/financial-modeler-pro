...
- **Module 1 (Project Setup / Costs / Financing)** — LOCKED at M2.0 Pass 58 (base). Funding Methods 2 + 3 calculate (commit `03a18ec`): real net-funding / cash-deficit totals in the Funding Requirement table + a "Funding Gap" sub-tab (`computeFundingGap`); each method has its own debt/equity % input (`netFundingConfig` / `cashDeficitConfig`). **2026-06-01 hotfix `5bfd144`:** Methods 2/3 size debt/equity from CAPEX at their own ratio (NOT a gap custom-curve); the gap drives only the requirement DISPLAY. The earlier custom-curve routing zeroed all debt + equity in the Cash Flow whenever the gap was not fed (always inside `computeFinancialsSnapshot`) — never set `customDebt/EquityByPeriod` for Methods 2/3 because an all-zero custom curve still trips `useCustom` in `debtEquity.ts`. Cash Sweep + Dividend waterfall live (Pass 2S → 2Z).
