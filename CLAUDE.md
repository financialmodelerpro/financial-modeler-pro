...
- **Module 1 (Project Setup / Costs / Financing)** — LOCKED at M2.0 Pass 58 (base). Funding Methods 2 + 3 calculate (commit `03a18ec`): real net-funding / cash-deficit totals + a "Funding Gap" sub-tab (`computeFundingGap`); each method has its own debt/equity % (`netFundingConfig` / `cashDeficitConfig`). **GAP-SIZED DRAWDOWN live (commit `7d340fc`, 2026-06-01):** Methods 2/3 size debt/equity to their NET requirement (M2 = capex net of pre-sales; M3 = the cash deficit to maintain min cash), split at the method's own ratio. The circular dependency (gap needs a full snapshot, debt sizing needs the gap) is broken by a GUARDED TWO-PASS in `computeFinancialsSnapshot`: pass 1 sizes from capex, then the gap is derived and the snapshot re-runs ONCE with it (only Methods 2/3; 1/4 single-pass). BS balances AND Direct CF == Indirect CF for all four methods. Sizing rule in `funding.ts`: set `customDebt/EquityByPeriod` ONLY when a real gap is fed; an all-zero custom curve still trips `useCustom` in `debtEquity.ts` and zeroes funding (the `5bfd144` regression), so the no-gap path falls back to capex sizing. Cash Sweep + Dividend waterfall live (Pass 2S → 2Z). **2026-06-02 — three segregated schedules + conditional IDC:** the Funding Gap sub-tab's single consolidated waterfall is split into three self-contained schedules, each ending in its own Closing Cash (chained: pre-distribution closing −sweep = post-sweep −dividend = final = Direct CF closing): (1) Method 3 Funding Requirement, (2) Debt Repayment via Cash Sweep (EXISTING loans first, then by priority), (3) Dividend via Cash Sweep (paid per policy even if debt not fully swept). `idcConfig.fundingMode: 'conditional'` is now the DEFAULT (2026-06-02): IDC is raised as new debt only to the extent needed to maintain minimum cash; in construction periods with surplus cash the interest is PAID IN CASH (still capitalised to the asset basis). Decided PER PERIOD: cash-short construction years still capitalise, surplus years pay cash (`'debt_drawdown'`/`'cash'` override explicitly). Schedule 1 is titled "Method 3 — Cash Deficit Funding" (matches the Financing-Inputs method name); its Min-Cash floor + "Cash Available for Debt + Dividend Payment" rows live in the Cash Sweep schedule (tier 2), not the funding-sizing schedule. Full engine integration via a per-tranche `interestCapitalizedCashPaid` reclassification (no new composer balance term: `debtServiceCash = accrued − capitalized + principal` auto-carries the cash outflow). The IDC drawdown ↔ finance cost ↔ debt balance circularity (the one Excel resolves with iterative calc) is solved by an **ITERATIVE FIXED-POINT loop** in `computeFinancialsSnapshot`: the inner `computeFinancialsSnapshotOnce` runs with the current `{fundingGap, idcCashBudget}`, `deriveCircularInputs` re-derives them from the result, and it re-runs until both stop changing (max per-period delta < 1, cap 25 iters). The same loop also converges the Methods 2/3 gap-sizing (replacing the old single re-pass). Each pass is internally consistent so BS balances + Direct==Indirect at every iteration; convergence pins drawdown + finance cost to their self-consistent values. Explicit-opts calls (verifiers) still do one pass. Verifiers: idc-depreciation §J + funding-methods [IDC] + [CONV] fixed-point + bs-reconciliation N5.
