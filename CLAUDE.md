...
- **Platform infra**: project-switch state-leak fixed; session-based versioning + per-version change_log; schema-tolerant reads; paginated version history. **PDF full-project report** (`lib/pdf/generateProjectPdf.ts` on pdf-lib) mirrors every module tab via shared `lib/reports/` builders (m4/opex/capex/financing/cos), with per-module part picker + number-scale + **version picker** in `ExportModal` (file named after the chosen version). **Excel MODEL export** (`lib/excel/buildModelWorkbook.ts`, formula-driven, true-mirror): Module 1 complete (Assumptions / Timeline / Land & Area / Capex / Financing), accounting number format (dash for zero, % 2dp), Calibri 9.5, frozen 4-row period header (Period 0 = opening Dec(startYear-1)), scale + decimals selector; verifier `verify-excel-export` (105). **Financing tab is a full detailed mirror on the LOCAL-INPUT pattern**: each tab pulls every assumption into a top "Inputs (linked from Assumptions)" block (link once), then all calcs reference the LOCAL cells (interest = (opening+draw)x$localRate), so the math audits within the sheet. Sections: Funding requirement (M1/2/3 + selected) / Total debt+equity required / per-facility debt movement / combined debt / finance cost / IDC allocation by asset / equity movement / IDC pool / Funding Gap (M2 + M3 waterfall) / Cash Sweep waterfall + dividends + per-tranche. Assumptions has a `FINANCING INPUTS` separator band dividing Capex inputs from Financing inputs. Module 1 audited end-to-end vs the engine under a rich fixture (sweep / dividends / existing facility / conditional IDC); the audit caught + fixed a per-facility "IDC capitalised (to debt)" zero-under-no-surplus bug (now a permanent verifier guard). When auditing financing, exercise capitalise/conditional IDC + sweep + dividends + an existing facility (the base verifier fixture is thin: Method 1, no sweep/dividends/IDC). **Auto-updating platform walkthrough guide** (`lib/guide/`, Topbar "Guide" button): in-app view + PDF/Markdown download, structure derived from the MODULES + MODULE_TABS registries.
