...
**Last updated: 2026-05-16. Module 1 (REFM) LOCKED at M2.0 Pass 58. Today's fine-tuning passes: 56 (Pre-Capex split into Land + Building/Infra per asset so future depreciation only charges the Building portion; sums via getAssetPreCapexTotal helper, additive migrateM20pass56SplitPreCapex seeds Building from legacy field); 57 (sub-unit Count column directly editable in Units mode so existing-operation assets can record Units/Keys without forced BUA / Unit Size; onEditAreaUnits no longer zeroes Count when Unit Size is missing); 58 (reconciler double-count fix for existing facility raised inside project axis: Pass 36 drawAsInflow moves openingBalance into drawSchedule[0] and zeroes initial bal, reconciler now mirrors that path via drawSchedule sum > 0 so expectedClosing no longer doubles). Prior session (Passes 37-55) themes still apply: Existing Operations panel collapsed INTO each facility's TrancheCard (per-phase BS + per-asset baseline below rate + repayment rows when phase is Operational); per-asset Existing Debt is sole input, facility Opening Balance auto-synced via useEffect; YoY editor spans full operations horizon when periods unset; Dashboard tile shows Pre-Capex headline (= Debt + Equity sublabel); Tab 1 Historical Baseline + Current Debt Outstanding removed (engine reads per-asset via existing.ts); migrationsApplied marker stops banner-on-every-reload; Dashboard redesigned with hero strip + 6 KPI tiles + 4-card module deck. Modeling Dashboard + REFM marketing DB-driven. **Module 1 status: LOCKED. Module 2 (Revenue) is next.** See CLAUDE-REFM.md.**
