...
**Last updated: 2026-05-13. Current REFM phase: M2.0 Pass 13 (Universal results-table layout + Tab 4 Financing restructure), 5 commits. Themes: universal prior-period column via shared `_shared/periodAxis.ts`; universal column-width consistency via new `COLUMN_WIDTHS` + `tableMinWidth` tokens + shared `<colgroup>` + `tableLayout:'fixed'` on every results table; Method 2 (Line-Item Based Financing) removed entirely with snapshot migration to Method 1; new Capex Breakdown table on top of Tab 4 Inputs; new two-rule Method 1 engine routing Land Cash per parcel funding type + new Total Debt Required + Total Equity Required tables replacing the old Inputs Summary collapsible; shared `_shared/GranularityRadioBar.tsx`. Layered on M2.0 Pass 12 + Pass 11 + Tab 3 Critical Regressions Round 2 + Tab 2 Pass 1-3. See CLAUDE-REFM.md.**
