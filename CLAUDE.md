...
- **Module 4 (Financial Statements)** — WIP at Pass 2M. Composer `computeFinancialsSnapshot` pulls every upstream engine and produces P&L + Direct CF + Indirect CF + BS + IDC allocation. Four sub-tabs in sidebar: Schedules (parent shell with Fixed Assets & D&A + BS Schedules sub-tabs) / P&L / Cash Flow / Balance Sheet. Pass 2j adds prior-year column. Pass 2k nests per-asset rows under strategy buckets. Pass 2L makes Revenue / CoS / Opex / Capex sections collapsible, replaces asset-filter dropdown with phase-filter buttons (P&L + Direct CF), and breaks debt financing into per-tranche lines. Pass 2M closes the audit gaps: opening cash input for operational phases (A1), Finance Cost split IDC vs Operations per tranche (A2), per-contribution Equity decomposition (A3), BS phase filter (B1), Indirect CF phase filter + decomposition (B2), shared `FAST_INPUT` constant in `_shared/inputStyles.ts` (B3), plus four new verifier scripts: `verify-cost-of-sales-v2` 21/21, `verify-idc-depreciation` 22/22, `verify-asset-cost-allocation` 14/14, `verify-m4-bs-reconciliation` 44/44. Total verifier sections: 440/440 green.
