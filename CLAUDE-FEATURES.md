...
| **Marketing Studio** | ✅ Complete (Phase 1.5) | Drag-and-drop canvas editor replaces fixed templates. `/admin/marketing-studio` — element-based design (text, image, shape) with `react-rnd` for drag + resize, undo/redo stack (50), keyboard shortcuts (delete, Ctrl+Z/Y, Ctrl+D, Ctrl+C/V, Arrow nudge). Custom dimensions. 5 starting presets: YouTube Thumbnail, LinkedIn Post, Instagram Post, Instagram Story, Blank Custom. Background: solid/gradient/image with overlay. Brand Kit expanded: primary logos + photo + additional_logos[], additional_photos[], uploaded_images[] libraries. Image elements pick from any Brand Kit asset. Canvas is WYSIWYG — download renders via satori `ImageResponse` at full resolution. AI captions extract text from elements (reading order). Migrations 100 + 101 |
