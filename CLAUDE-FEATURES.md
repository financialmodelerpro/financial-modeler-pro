...
|---------|--------|-------|
| **ONE cost-of-sales engine, owned by Module 2 (2026-08-30)** | ✅ Complete | Cost of sales was computed by TWO engines with different bases: the P&L used `buildCostOfSales`, the Module 2 screen and both exports used `buildCostOfSalesV2` on a base each surface re-assembled by hand. Measured **407,131,731 apart in one year** while agreeing to 0.24% over a lifetime, which is what hid it. v1 won (its spread is on the exact series the P&L books as revenue, so gross margin stays constant per asset; v2's swung 75.7% -> 6.4% -> 27.6%). `lib/costOfSales.ts` now assembles base + IDC + spread + inventory + vintage ONCE; every surface reads `snap.byAssetCostOfSales`. v2 DELETED. Two base defects fixed at source: a hand-rolled Y0 rule dropping 56,375,000 off the axis, and a silent truncation making `computeAssetCost.total` disagree with the sum of its own periods. Cost of sales now ties to the P&L in EVERY period on both live projects and inventory closes at 0. CHANGELOG 2026-08-30o, TRAPS 7.31. NOT browser-verified. |
| **Portfolio Dashboard (2026-08-30)** | ✅ Complete | Replaced five count tiles (three of which read a workflow status nothing in the product ever sets, so they were permanently zero) with real engine-derived money: GDV, development cost, funding requirement, peak debt, portfolio equity IRR, equity multiple. `GET /api/refm/portfolio` runs the real engine per project on the **Management base case**; `portfolioMetrics.ts` aggregates with the rules that make it non-trivial (IRR computed ONCE over summed streams and labelled as such, peak debt as max of the SUMMED series, DENSE calendar axis, per-currency totals never summed across currencies, un-modelled projects counted OUT). The funding-by-year chart moved to the single-project Overview where amounts fit on the bars. Archived excluded. |
| **Project soft delete, archive unchanged (2026-08-30, mig 224)** | ✅ Complete | Delete stops being permanent: hidden, un-openable, out of the cap, hard deleted after 30 days by the existing daily cron. Archive is untouched (visible, user-reversible, never expires). Restore is ADMIN ONLY via the Projects Browser's Deleted bin; read-only grace now blocks deletion. `RETENTION_DAYS` defined once; registry-driven so ERM/BVM inherit it. The native `confirm()` pair replaced by one modal. |
| **Admin email campaigns (2026-08-30, migs 225 + 226)** | ✅ Complete | `/admin/campaigns`: audience -> template -> preview -> confirm, over the EXISTING Brevo path. Each rule lives once (`src/shared/email/campaigns.ts`): admins excluded unless explicitly picked, unsubscribed always excluded and reported, every merge value escaped once, per-user HMAC unsubscribe with nothing stored, one log row per recipient including failures. The send re-resolves the audience and refuses if the count moved since the preview. Three defects from the first real send fixed same day (dead CTA from a blank link field, booking-page default, Training Hub footer copy on a Modeling Hub email). |
| **Account deletion, both sides + access requests (2026-08-30, migs 219 + 221)** | ✅ Complete | ONE deletion engine (`src/shared/account/deleteUserAccount.ts`) for admin and self-delete: retains the financial ledger, refuses to orphan a live Paddle subscription, audits into `account_deletions`. A live FK blocked the first attempt because `admin_audit_log.target_user_id` is NO ACTION on prod, not the SET NULL migration 007 declares (TRAPS 2.9); the engine nulls it in code and mig 221 aligns the constraint. Dashboard leads with an access card for no-plan users + a once-only two-day reminder email. |
| **Password reset, rebuilt end to end (2026-08-30, mig 222)** | ✅ Complete | Every Modeling Hub password reset had failed silently since launch: mig 008 declares `password_reset_tokens` but the table never existed on prod, and the flow's own errors were unchecked so the user saw a false success. Rebuilt with token expiry, atomic single use, honest send failures and escaped values; proven live end to end on the deployed endpoint. |
| **One certificate system (2026-08-30, mig 223)** | ✅ Complete | The legacy `/api/training/certificates` endpoint read a dead table and returned empty for every student. Retired: six files deleted, five legacy tables deprecated in place. The live system is `student_certificates` + the `certificates` STORAGE BUCKET, whose name collision with the dead table had over-counted readers 10-to-3 in the diagnosis (TRAPS 2.10). |
| **Admin Projects Browser rebuilt on a registry (2026-08-30, mig 220)** | ✅ Complete | Repointed off the empty legacy `projects` table onto `PROJECT_SOURCES` (`src/shared/admin/projectSources.ts`), THE one place a project table is named, so ERM and BVM join as one entry each. Legacy table deprecated, not dropped. The admin user list's project count, the per-user read-only project modal and the stale pre-launch banner were fixed in the same pass. |
| **Platform guide rebuild + guided tour + first-run prompt (2026-08-20)** | ✅ Complete | Full-screen guide overlay (side nav, search, jump-to-module), ONE content source `lib/guide/guideContent.ts` rewritten to the current platform, tab registry moved to pure `lib/moduleTabs.ts` so the verifier measures the guide against the LIVE registry (the frozen-copy trap that hid two missing modules is closed). 34-step tour renders the guide text VERBATIM (string identity pinned), auto-runs once per user (mig 217 + localStorage fallback, MERGING writes), module-only walk with no project open, per-tab DOM emptiness skip, first-run "read the guide" prompt before the tour offer, both re-launchable from inside the guide. `verify-platform-guide` 150, `verify-platform-tour` 180. Narrative: CLAUDE-REFM.md 2026-08-20g/h. |
| **Trial export watermark + per-plan export gates (2026-08-20)** | ✅ Complete | Server-resolved diagonal stamp + footer on every PDF page (full, summary, IC deck) for watermark-listed plans; admin control on /admin/plans (text / on-off / plan list, default ON for trial only); paid exports byte-identical; failed resolution REFUSES rather than emitting unmarked. Deck route now enforces Module 7 per plan server-side (was UI-only; a trial hitting the endpoint got an editable .pptx). Excel gates on `excel_snapshot` (was `excel_formula` = false on all plans, denying pro/firm a sold export; trial stays denied). `verify-export-watermark` 84. |
| **OpEx zero seeds (2026-08-20)** | ✅ Complete | All 26 seeded values across the HQ / hospitality / lease default builders are ZERO; line structure intact. The engine fell back to the builders, so a new project silently carried 109.065m of HQ opex. Stored-but-disabled seed values cleared on hydrate (exact category+mode+value match; user-typed values preserved); NO number moved on any live project, proven. Frozen `LEGACY_OPEX_SEED_VALUES` is a record, not a source. `verify-opex-seed-zero` 45. |
| **Trial request lifecycle: detail card, decline email, approvable declines (2026-08-20)** | ✅ Complete | Request card shows the whole person (shared `signupProfile.ts` builder, also on the admin user record; Registered vs Requested both labelled). Decline sends a short neutral email (per-request dedupe, never-throws). Declined rows stay visible in their own section and CAN be approved later; history preserved via mig 218 (`declined_at/by` survive the approval). `verify-registration-qualification` 116, `verify-subscription-emails` 122. |
| **Registration success screen + light theme default + country combobox (2026-08-20)** | ✅ Complete | On success the form is REPLACED by a confirmation screen (address named, activation step, trial-approval step, resend via the dedicated endpoint); confirmation email confirmed sending (`confirmEmailTemplate`, proven end to end by the test account). Dashboard dark default came from the OS `prefers-color-scheme`; removed, no stored preference now = LIGHT, stored choices untouched. Create Account country field rebuilt as a real combobox (`CountryCombobox`, one list, any-part filtering, arrows/Enter/Escape, storage byte-identical); PhoneInput gained the same arrow contract; signin register tab uses the same component. |
| **Topbar: RBAC badge parked for Module 8 + contrast sweep (2026-08-20)** | ✅ Complete | Role badge, tooltip and RbacModal mount removed from the surface (role LOGIC untouched; modal file kept on disk for the M8 restore; the sidebar indicator, the one other reader, is display-only now). Guide / Save-as-new-version / Colours all rendered black on the navy bar (bare `.pm-btn` has no colour); all take the `--color-on-primary-navy` token trio, and the verifier sweeps every bare pm-btn as a CLASS. |
| **AI foundation, Unit 1 central client (2026-07-29)** | 🟡 1 of 9 units | Platform-agnostic AI engine per [docs/AI_FOUNDATION_GUIDELINE.md](docs/AI_FOUNDATION_GUIDELINE.md). `src/shared/ai/` is the single call path: `runAi()` is the only read of `ANTHROPIC_API_KEY` and the only SDK client construction, never throws (typed `AiResult` union), and centralises model/timeout/retry config. Migrated the ONE pre-existing AI call site (`/api/admin/newsletter/enhance`) and removed its retired model pin. `verify-ai-client` 49. **BLOCKED on the Unit 0 live-key test** (no key reachable outside Vercel env). Units 2/3/5 need migrations from **201**. |
| **My Model student view + reviewed-file on reject (2026-07-29)** | ✅ Complete | New sidebar entry in the training dashboard: per course, EVERY submission attempt with status, reviewer comment, and both halves side by side (the student's file + the marked-up model returned). Previously the reviewed model was reachable only inside a course tab, only in the approved state, and only for the LATEST attempt, so earlier feedback vanished on resubmission. **The marked-up model now returns on EITHER decision** (was `decision === 'approve' && reviewedFile` in the route and multipart-only-on-approve in the admin UI, which withheld feedback from exactly the students being sent back for another attempt); the rejected email gained the download block. Two new ownership-checked student routes (`.../history`, `.../[id]/file`) that never leak storage paths. Both review emails deep-link `?tab=my-model`. No migration (mig 185 columns already existed). `verify-my-model-view` 41. |
| **Testimonial company byline + Modeling share button (2026-07-29)** | ✅ Complete | Company was already captured and stored by BOTH systems (`testimonials.company` mig 009, `student_testimonials.company` mig 021) and written by every route, so **no migration was needed**; the gap was display. New `testimonialByline()` renders "{title} at {company}", falling back to just the title, or just the company with no leading "at" (so rows without a company render exactly as before, never a dangling "at"). Modeling Hub dashboard gained its own "Share your experience" button. **Fixed a Modeling submit path that could never have worked**: the shared modal posted `{type, content, videoUrl, jobTitle, linkedinUrl}` while the route reads `{testimonial_type, text, video_url, role, linkedin_url}`, so every modeling submission would have 400'd; invisible because nothing mounted the modal in modeling mode. `verify-testimonial-company` 62. |
| **LinkedIn URL normalization (2026-07-29)** | ✅ Complete | A student submitted `www.linkedin.com/in/name` with no scheme; a bare host in an `href` is a RELATIVE path, so the button opened `learn.financialmodelerpro.com/www.linkedin.com/in/name`. New `src/shared/utils/externalUrl.ts` normalizes at BOTH ends: render (fixes rows already stored, no migration) and submit (keeps new data clean). Rejects `javascript:`/`data:`/`vbscript:`/`file:`, and returns null for anything that cannot become absolute so a bad paste renders no link rather than one pointing back at us. Applied to testimonial cards (admin, training carousel, portal, modeling) and instructor links. `verify-external-url` 27. |
| **Testimonial review modal (2026-07-29)** | ✅ Complete | The admin testimonials table truncated content at 90 chars with only a `title` tooltip, so a long testimonial could not be read before approving. Click-to-open modal shows the full text with line breaks preserved plus all metadata, and Approve / Reject / Reset / Visible / Feature act from inside it. Keyed on `{id, source}` so each action refetches and the open modal reflects the update. No API change (the route already returned full text). |
| **Certificate gate vs model rejection (2026-07-29, diagnosis)** | ✅ Verified correct | Confirmed a REJECTED model blocks the certificate on every automatic path: the inline trigger is gated twice (`withholdFinalResult` + the engine), and "Issue Now" fails with `model_not_approved`. Live prod state: both `model_submission_required_*` flags `"true"`, zero certificates held by any student without an approved model. **Force-Issue overrides BOTH gates** (model approval AND exam eligibility, since the route never calls `checkEligibility` and defaults the final score to 0) and its confirm dialog said only "bypasses the watch-threshold check" , a check that **does not exist on the certificate path** at all. Dialog + two stale docstrings corrected. `diagnose_cert_model_gate.ts` added. **Open policy question: `hasApproved` is sticky**, so an approved student keeps their certificate if a later resubmission is rejected. |
...
| **ERFM foundation (2026-09-21, Unit 1)** | ✅ Complete | Authenticated `/erm` route, isolated platform placeholder, and the locked ERFM master roadmap are in place. The foundation has no financial data, schema, API, provider, report, billing or Modeling Hub catalog integration. Future units require explicit approval. |
...
| **Email System** | ✅ Complete | Brevo (`@getbrevo/brevo` v5, migrated from Resend on 2026-05-11 commit `166a8ec`), 11 templates all using `baseLayoutBranded()` (sender-agnostic; templates were untouched in the migration). All 3 remaining triggers migrated from Apps Script to Next.js: quizResult (submit-assessment, now FINAL EXAM ONLY — per-session emails removed in the same Brevo commit; per-session results are dashboard-only), registrationConfirmation (confirm-email), lockedOut (submit-assessment when max attempts, still fires per-session). `/api/email/send` (the last Apps Script email bridge) was DELETED 2026-07-31: it had no caller, its templates were all already sent natively, and with RESEND_WEBHOOK_SECRET unset in Vercel it was an unauthenticated relay in production. **Admin Communications** (`/admin/training-hub/communications` Send Campaign) goes through `sendEmailBatch` + `baseLayoutBranded()` (Brevo Promise.allSettled loop with binary ok/fail semantics; previously Resend `batch.send`). Per-recipient outcomes drive the `training_email_log.status` column (sent / failed). `{name}` token in the body resolves to the recipient's first name. |
