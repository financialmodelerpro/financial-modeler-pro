...
| **Marketing Studio** | ✅ Complete (Phase 2) | Drag-and-drop canvas editor + multi-platform workflow. 8 presets including 3 FMP-branded (YouTube 1280×720, LinkedIn 1200×627, Instagram 1080×1080). **Quick Fill**: auto-populate text elements from articles / live sessions / training sessions (`COURSES` config) via id-prefix matching. **Multi-platform captions**: one "Generate All" button runs parallel Claude calls for LinkedIn / Instagram / Facebook / WhatsApp / Twitter / YouTube with per-platform prompts + tone selector (Professional / Casual / Thought Leader / Educational). Captions shown in per-platform tabs with editable textareas + copy buttons, persisted to `marketing_designs.ai_captions`. **Export to All Platforms**: ZIP download of FMP YouTube + LinkedIn + Instagram adaptations (text carries across, layouts rebuilt per preset). Saved designs sidebar shows live thumbnails + template-type filter. Canvas is WYSIWYG with `react-rnd` drag/resize, undo/redo, keyboard shortcuts, aspect-ratio lock, background library. Migrations 100 + 101 + 102 |
