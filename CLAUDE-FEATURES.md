...
| **CMS — Dynamic Page Builder** | ✅ Complete | 11 section types, drag-and-drop, SEO. **All system pages** now CMS-driven (migrations 044-051): about, contact, pricing, training, modeling, home, articles, training-sessions. Each page fetches `page_sections` and renders via SectionRenderer + inline dynamic components for `_dynamic` markers. Hardcoded JSX retained as fallback if no CMS sections exist. Prior incident: `fdaa77d` added CMS conditional + seeded empty placeholder rows, causing blank pages; fixed in `7da9a2e`. Dynamic markers: `pricing_plans`, `pricing_faq`, `courses`, `testimonials`, `upcoming_sessions`, `modules`, `founder`, `articles`, `pricing_preview`, `live_sessions`, `contact_body` |
