...
| **CMS — Dynamic Page Builder** | ✅ Complete | 21 section types, drag-and-drop, SEO, per-field visibility checkboxes, per-field width/alignment controls. **Home page** uses Option B approach: each section fetched from `page_sections` and fed into custom hardcoded JSX (not SectionRenderer) for pixel-perfect control. Sections: hero (053), stats (054), mission/vision (055-056), what-is-fmp (056-057), two-platforms (058), founder (059-063), pacemakers (062). All sections respect `visible=false` eye icon. `getAllPageSections()` fetches without visible filter for hide/show support. **Training page** also uses Option B (migration 065): 9 sections with `getAllPageSections('training')`, each section CMS-editable with hardcoded fallbacks, dynamic markers for courses/upcoming-sessions/testimonials. **Modeling page** also uses Option B (migration 070): 7 sections with `getAllPageSections('modeling')`, dynamic markers for modules/testimonials, audience cards array. Other pages (about, contact, pricing, articles, training-sessions) use SectionRenderer + `_dynamic` markers. Editors: SmartColumnsEditor (auto-detects TwoPlatforms vs PaceMakers vs generic), SmartTeamEditor (auto-detects Founder vs generic), FounderEditor (6 sections: home card, credentials, photo, buttons, full profile, booking page), PaceMakersEditor (logo, badge, heading, description, CTA, services with drag-reorder). TextImageSection supports checklist items, background images with padding/position/fit controls, image upload via MediaPicker |
