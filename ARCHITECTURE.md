...
- **J.10, Centralize admin auth into middleware**. Every `app/api/admin/*` route currently re-runs `getServerSession(authOptions)` + role check at the top. Estimated 1 day; deferred from Phase 2.x because it changes auth posture and warrants its own phase. **Read this before starting: `src/middleware.ts` has never executed in production** (Next resolves middleware at the repo ROOT, and there is no `middleware.ts` there, so the file is never compiled). Hoisting the per-route checks into it as written would therefore DELETE the only server-side enforcement that exists. The move to the root has to come first, and it has its own prerequisite (verify the NextAuth JWT `role` claim shape against a live token, or activating the gate locks every admin out). See PLATFORM_INVENTORY.md Section 10 and CLAUDE-TODO.md.
- **CLAUDE.md size: CLOSED 2026-08-30.** It reached 166.8 KB against a 150 KB harness limit and was restructured: CLAUDE.md now holds only standing rules, module status, one-line invariants and pointers (~40 KB), every dated session bullet moved VERBATIM to a new [CHANGELOG.md](CHANGELOG.md) (newest first, nothing lost), and `scripts/verify-md-size.ts` WARNS above 100,000 chars and FAILS above 140,000 so it cannot creep back unnoticed. The placement rule ("dated narrative to CHANGELOG, one-line invariants only to CLAUDE.md") is written into CLAUDE.md's end-of-session rule.
