...
**The hub-side passes from the evening of 2026-08-20, each deployed same day. Compact; commits carry the detail.**

**(1) TRIAL REQUEST CARD SHOWS THE WHOLE PERSON (`1cf53356`).** Name leads (email beneath), then phone / city / country / company / job title, the qualification badge, the note IN FULL, and BOTH timestamps labelled: **Registered** (users.created_at) and **Requested** (the request row's own created_at, fetched all along and shown nowhere). ONE implementation: `src/shared/admin/signupProfile.ts` owns field order, country resolution (the shared list, not a second rule) and the UTC stamp format; the request card AND the admin user record render from it. The mig-216 schema retry still tests ONLY its two columns, so a missing contact column can never downgrade the queue. `verify-registration-qualification` section F.

**(2) A DECLINE IS ANSWERED, AND IS REVERSIBLE (`0636d130`, `5504d95a`, mig 218 APPLIED).** `trialDeclinedEmail`: short, neutral, no reason, no admin text, pricing link, no-reply@, name escaped, same never-throws dispatch, deduped PER REQUEST (`evt:<requestId>`) so a re-decline sends once while a later new request gets its own answer; wired AFTER the status update so the email never reports an unrecorded decision. Declined rows stay VISIBLE in their own amber section (own query, capped 25, a failure there cannot empty the pending list) with ONE action, Approve now; approve is allowed on pending AND declined, decline stays pending-only. History never overwritten: `declined_at`/`declined_by` keep the decline forever while `decided_*` takes the approval, with the pre-218 backfill at approve time. The decline email cannot block the approval email (distinct dedupe types; the approval keys on the fresh trial end date), pinned as checks. `verify-registration-qualification` section G, `verify-subscription-emails` 122.

**(3) SUCCESS REPLACES THE FORM (`9cf2994b`).** The success banner used to render ABOVE a still-populated form, reading as if nothing happened. On success the form is GONE: a confirmation screen states the account is created, names the ADDRESS the confirmation email went to, says the link must be clicked to activate, that the trial is then requested from the dashboard and goes to the team for approval, and offers resend through the dedicated `/api/auth/resend-confirmation` (captcha tokens are single use, so re-POSTing the form was never an option). **The confirmation email is confirmed sending**: `confirmEmailTemplate`, no-reply@, and the test account (created 14:40 UTC, `email_confirmed: true`, a state only the emailed link sets) proves send -> deliver -> click end to end. The verifier pins the render CONDITION (`if (success) {`), not just the markup (TRAPS 10.11).

**(4) LIGHT IS THE DEFAULT (`9cf2994b`).** The dashboard's dark default came from the OPERATING SYSTEM: no stored `modelingDarkMode` fell back to `prefers-color-scheme`, so a new user on a dark-OS machine opened dark having chosen nothing. Fallback removed: no stored preference means LIGHT; a stored choice keeps winning, so no chosen theme moved. The platform workspace already defaulted light and is pinned.

**(5) THE COUNTRY FIELD IS A REAL COMBOBOX (`db5e637e`).** The Create Account field was an `<input list>` + `<datalist>`, whose popup is BROWSER-OWNED (type-ahead jumping, no arrow contract, unstylable; TRAPS 6.3), so this was a rebuild: `src/shared/components/CountryCombobox.tsx`, any-part case-insensitive filtering ("kist" finds Pakistan), visible arrow highlight, Enter/Escape, click opens all 249 entries, ONE list (imports `src/core/countries.ts`), and STORAGE BYTE-IDENTICAL to the old path (code for a recognised pick, free text kept on blur) so every stored value keeps working. The signin page's embedded register tab (plain free text, no list at all) now uses the same component. `PhoneInput` did not jump but lacked arrows; it now has the same arrow contract with a visible, mouse-tracked highlight.
