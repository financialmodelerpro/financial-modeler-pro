 ...
 * past unrelated content to read the formula). Uses bounding boxes.
 */
async function assertProximate(page: Page, inputSelector: string, formulaTestId: string, label: string): Promise<void> {
  ...
  expect(
    verticalDistance,
    `${label}: formula ${formulaTestId} sits ${verticalDistance}px below ${inputSelector}, must be < ${PROXIMITY_PX}px (and >= 0 so caption is below the input).`,
