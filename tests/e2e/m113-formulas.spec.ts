/**
 * tests/e2e/m113-formulas.spec.ts
 *
 * M1.13 regression guard. Two contracts:
 *
 *   Spec 1: Every Module 1 tab carries inline plain-English formula
 *           captions adjacent to derived outputs. Walks Schedule ->
 *           Build Program -> Dev Costs -> Financing and asserts
 *           specific FormulaCaption testIds + a non-trivial number of
 *           data-formula="true" markers per tab.
 *
 *   Spec 2: Live recompute. Edit a Plot input on Build Program and
 *           verify the corresponding formula caption text updates
 *           in place without unmount, exercising the no-layout-reflow
 *           contract.
 *
 *   Light + dark screenshots, every tab, saved into
 *   tests/screenshots/M1.13/.
 */
...
async function createWizardProject(page: Page, name: string): Promise<void> {
  ...
  // Step 2: accept defaults (1 parcel seed) and continue.
...
test.describe('M1.13 inline plain-English formulas across Module 1', () => {
  ...
  test('Every tab surfaces FormulaCaption rows adjacent to derived outputs + live recompute', async ({ page }) => {
    ...
    await createWizardProject(page, 'M113-Formulas-AllTabs');
    const main = page.getByRole('main');

    // ── Schedule tab: timeline-summary captions present. ───────────────
    const tlEnd = page.getByTestId('timeline-formula-end');
    const tlTotal = page.getByTestId('timeline-formula-total-periods');
    const tlType = page.getByTestId('timeline-formula-type');
    await expect(tlEnd).toBeVisible();
    await expect(tlTotal).toBeVisible();
    await expect(tlType).toBeVisible();
    await expect(tlTotal).toContainText('Construction + Operations - Overlap');
    await page.screenshot({ path: resolve(SCREENSHOT_DIR, 'light-schedule.png'), fullPage: true });

    // ── Build Program tab: envelope, cascade, parking captions. ───────
    ...
    const plotIdAttr = await firstPlotCard.getAttribute('data-testid');
    ...
    // Live recompute: edit Max FAR + Plot Area, assert caption updates
    // in place. Exercises the no-layout-reflow contract.
    const farInput = page.locator(`#plot-${id}-maxFAR`);
    await farInput.fill('5');
    await expect(maxGfaRow).toContainText('* 5 =', { timeout: 3_000 });
    const areaInput = page.locator(`#plot-${id}-plotArea`);
    await areaInput.fill('200000');
    await expect(maxGfaRow).toContainText('200,000 * 5', { timeout: 3_000 });

    // Land Parcels block totals captions exist on Build Program.
    await expect(page.getByTestId('parcel-formula-area').first()).toBeVisible();
    await expect(page.getByTestId('parcel-formula-value').first()).toBeVisible();
    await expect(page.getByTestId('parcel-formula-cash').first()).toBeVisible();

    // Count of data-formula markers on Build Program. With one plot
    // (10 envelope + 5 parking + 3 parcels = 18 minimum) and any
    // assets bound (8 cascade per asset) we expect a comfortable
    // double-digit count.
    const bpFormulaCount = await page.locator('[data-formula="true"]').count();
    expect(bpFormulaCount).toBeGreaterThanOrEqual(10);
    await page.screenshot({ path: resolve(SCREENSHOT_DIR, 'light-build-program.png'), fullPage: true });

    // ── Dev Costs tab: per-row formula captions + grand total. ────────
    await main.getByRole('button', { name: '3. Dev Costs' }).first().click();
    await page.waitForTimeout(300);
    const costsFormulaCount = await page.locator('[data-formula="true"]').count();
    expect(costsFormulaCount).toBeGreaterThanOrEqual(5);
    await page.screenshot({ path: resolve(SCREENSHOT_DIR, 'light-dev-costs.png'), fullPage: true });

    // ── Financing tab: debt-equity, periodic-rate, repayment + summary. ─
