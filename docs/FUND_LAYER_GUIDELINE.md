...
**Two more things Step 7 established that are easy to misread as defects.** The balance sheet carries a worst residue of 360.8 currency units on a 7,034m sheet (5.1e-8 relative), and it is IDENTICAL with the fund layer off and on, so it is not fund related; the verifier asserts a relative tolerance AND that the fund layer does not worsen it. **CORRECTED 2026-08-18: attributing that residue to solver convergence was WRONG, and the diagnosis is worth remembering. It was a real defect** (in-kind equity was recognised at the OWNING parcel's phase while the in-kind land was capitalised in the CONSUMING asset's phase, and a parcel is project-wide), and the residue is now **0.00** on this project and on the other live one. Reading a small residue as numerical noise because it is small, and because it does not move with the feature under test, is how a genuine imbalance survives a verifier that was looking somewhere else. See docs/TRAPS.md 7.22. And the waterfall's `unallocated` reads -9.5e-7 against 4,437m of distributions, a relative 2.2e-16, which is one machine epsilon: "zero by construction" is true in exact arithmetic, so the check is relative to the cash being split rather than literal zero.
