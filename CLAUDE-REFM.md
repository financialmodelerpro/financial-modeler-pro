...
- **Full-project PDF export (`5db031d`):** `lib/pdf/generateProjectPdf.ts` (pdf-lib). Cover (name/version/date/KPIs) + one navy-headed section per selected module (M1 overview/funding/phases/tranches/cash-waterfall, M2 revenue+CoS, M3 opex, M4 P&L/CF/BS, M5 returns + sponsor streams). Landscape A4, wide period tables split across pages (label+Total + header repeat per column-chunk + on overflow), footer = page/project/version. Pure (reads the snapshots, no new calc). `ExportModal` is a two-step flow (format → registry-driven module checkboxes → Generate → Blob download); pdf-lib lazy-imported. Verifier `verify-pdf-export.ts` (7/7). NEXT: Excel export, same structure.
