...
**Last updated: 2026-05-12, M2.0 Costs Cleanup Pass 10 partial ships. 8 of 10 fixes shipped (regression audit + Fix 6 collapse foundation + Fix 1 readonly collapsed cells + Fix 9 deeper Land Zero fallback + Fix 2 addAsset cost line auto-replication + Fix 5 NDA Recon polish + Fix 7 Revenue in sub-unit summary + Fix 4 Sell+Manage Operate companion auto-creation + Fix 10 commission revenue hooks). Fix 3 (hybrid project-wide + per-asset override architecture) and Fix 8 (universal AccountingNumberInput sweep) DEFERRED to follow-up session; both are large-scope changes. Verifier verify-m20costsCleanup-pass10.ts 43/0/0 across 10 sections. Schema stays v8 additive (Asset gains parentAssetId / isCompanion / companionType / unitsFromParent; CostMethod gains percent_of_revenue_cash / _sale; FinancingDataHooks gains getTotalRevenueCashBasis / _SaleBasis).**
...
- **Fix 3 (hybrid project-wide + per-asset override architecture, DEFERRED):** Largest scope of Pass 10. Walks back Pass 7's per-asset CostLine architecture to a hybrid: single project-wide `Project.costLines[]` master table; each asset's Total recomputes per its own metrics on pill switch; lightweight per-asset Override toggle re-introduces `CostOverride[]` (NOT the Pass 4 stacked master+replica double-table). Migration requires merging per-asset replicas back into single project-wide lines with first-asset-wins on conflict + banner. Deferred to follow-up session due to scope; user explicitly approved hybrid approach (memory `project_m20_pass10_decisions.md`).

- **Fix 8 (universal AccountingNumberInput sweep, DEFERRED):** ~25 bare `<input type="number">` sites in Tab 2 (parcel area + rate, sub-unit area + unit size + rate, asset BUA/GFA/Support/Parking, management agreement %, useful life) and additional sites in Tab 1 + Tab 4 lack thousand-separator formatting. Sweep wraps each money/area input in AccountingNumberInput (focus = raw, blur = commas). Percentages + integer period numbers stay bare. Deferred to follow-up session as a focused sweep commit.

- **No schema migration banner:** All Pass 10 fixes are additive (new optional Asset fields, new CostMethod values, new hook signatures, new UI display behavior). Existing snapshots load without a migration step. Fix 3's hybrid revert WILL ship with a migration banner when it lands.

- **Verifier:** `scripts/verify-m20costsCleanup-pass10.ts` 43 pass / 0 fail / 0 skip across 10 sections (audit doc presence + each fix's source markers + calc fixture math + em-dash sweep + Pass 10 status in CLAUDE-REFM.md).

Commits (9): `5752454` (audit), `ca4c5ab` (Fix 6 collapse foundation), `2cff997` (Fix 1 readonly collapsed cells), `162d053` (Fix 9 deeper Land Zero fallback), `dfec7c7` (Fix 2 addAsset auto-replicate + Asset schema additions), `aaf847b` (Fix 5 + 7 NDA Recon polish + Revenue in summary), `fc63dfa` (Fix 4 Sell+Manage companion), `56f0f12` (Fix 10 commission revenue hooks), closure (verifier + CLAUDE-REFM.md update). Type-check clean on every commit.
