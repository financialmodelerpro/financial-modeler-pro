...
**Last updated: 2026-05-13, M2.0 Pass 11 (Tab 3 Costs + Results polish). 11 commits over a single session: P11 Fix 1 (per-phase copy panel, superseded), Fix 2 (Results Table 1 footer + per-line rows now apply `phaseStartYear - projectStartYear` offset so Project Total columns agree with the per-asset rows summing into them), Fix 3 (copy panel lifted to project level above the phase filter, target list = `allVisibleAssets` grouped by phase, cross-phase apply matches lines by case-insensitive name since master cost line ids are phase-scoped; also drops two dead Expand all / Collapse all bulk buttons left behind after T3-edit-runtime v4 deleted per-row collapse state), Fix 4 (`SummaryTables` walks every in-scope asset's `bd.perPeriod` to compute min/max non-zero annual indices then `cropRow<T>(arr)` slices header, body, footer + `colSpan` so trailing zero columns and empty leading years disappear; Tab 4 Financing's independent `activePeriods` cropping unaffected), Fix 5 (source asset is now a `<select>` grouped by phase, decoupling the panel from the pill-bar `activeAsset`; switching source mid-flow drops its id from the target set), Fix 6 (DIAGNOSTIC + FIX: engine extends `AssetCostBreakdown` with `perLinePerPeriod: Record<lineId, number[]>` filled by the same `distributeItemCost` call that already feeds `perPeriod` + land splits, Module1Costs Table 1 per-line nested rows consume it and lift onto the project axis via the same offset as the asset row; the smear-by-asset-curve fallback was destroying manual % and single-period line phasing for multi-line assets), Fix 7 (Apply panel now writes a FULL `CostOverride` on source AND every target with cloned `distribution[]` / `perSubUnitRates{}`; `removeCostOverride` path on apply is gone so master edits stop cascading - one-time deep clone, not a live link), Fix 8 (`isStartEndLocked = false` constant so Start/End is universally editable across Land + Auto-IDC + every other line; Auto-IDC's Value + Method stay locked since they flow from the financing facility), Fix 9 (Combined view splits each asset's Table 1 group into header row [asset name only, full `colSpan`, navy 12% fill, no values] → per-line nested rows → "Asset Subtotal" closing row), Fix 10 (`resolveAssetAreaMetrics` switches `bua` / `nsa` / `gfa` to `Math.max(hierarchy, asset-level)` so stub sub-units no longer drown out a real `asset.buaSqm`; consistent across every phase), Fix 11 (Single Asset view + Tables 2/3/4 in Single Asset adopt the same header + subtotal pattern, subtotal label = `{name} Subtotal`; Combined keeps the simple `{name}` label on Tables 2/3/4 to avoid noise). Diagnostic doc `docs/m20-t3-perline-divergence-diag.md` archives the engine-vs-UI divergence audit feeding Fix 6.
