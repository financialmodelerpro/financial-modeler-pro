...
**Last updated: 2026-05-25. Module 1 LOCKED at M2.0 Pass 58 (base). Module 2 LOCKED at Pass 9N. Module 3 LOCKED at Pass 5d. Module 4 WIP at Pass 2Z (consolidated Cash Sweep + Dividend master waterfall, per-parcel in-kind stamping, IDC column alignment, Phase 1 i=0 rescue). Verifiers: revenue 133/133; escrow 46/46; opex 38/38; opex-ap 24/24; fixed-assets 82/82; phase-date-preservation 37/37; phase-date-scenarios 7/7; cost-of-sales-v2 24/24; idc-depreciation 114/114; asset-cost-allocation 14/14; m4-bs-reconciliation 174/174 — 693 sections green across 11 scripts.**

**2026-05-25 UI convention (commit `8fe9ceb`):** universal prior-year column. Every results table now leads with a prior-year column (= projectStartYear − 1); Module 2 (Revenue Output / CoS / Schedules / Escrow) + Module 3 (Opex Output) extended to match Module 1 + Module 4 via auto-derive in each local `PeriodTable` + the shared `VintageMatrix`. Same commit: `VintageMatrix` row-total header "Cohort Total" → "Total"; removed the "Cum % Sold" trailing column from the Revenue 1c table. Render-layer only, engine arrays + verifiers unchanged. See the "Prior-year column convention" bullet in Module 1 Conventions below.

## Pass 2O → 2Z narrative (2026-05-24 session)

**Pass 2O — IDC policy split.** `Project.idcConfig = { allocationBasis: 'land' | 'bua', capitalize: boolean, fundingMode: 'debt_drawdown' | 'cash' }`. Engine (`schedule.ts`) decouples accounting from funding via new `interestForAssetBasis` field on `FacilityResult`; existing `interestCapitalized` keeps its meaning as "amount added to debt balance" (funding side). `combineDebtService.totalInterestExpensed = totalInterestAccrued − totalInterestForAssetBasis` (accrual basis). New `interestDuringConstruction` per-facility array. Composer extracted `computeIdcSnapshot` standalone helper. UI moved from BS Schedules MEMO → Module 1 Financing (Inputs: "1b. IDC Policy" 3 pill toggles; Schedules: "IDC Allocation — by Asset" with basis/capitalize/funding chips + per-asset summary + routed-to-CoS / routed-to-FA sub-tables).

**Pass 2P — BS Equity split + RE Schedule + CF drop in-kind.** BS Schedules E1 splits Equity into separate rows (Cash / In-Kind / Existing). New BS Schedules E2 Retained Earnings Roll-Forward (Opening + PAT − Reserve − Dividends = Closing). CF financing block: drop in-kind from drawdown (cash-only); in-kind as `(memo)` row. `cashFromInv` switches from `inclAllLand` → `exclLandInKind`; `cashFromFin` from `equityDraws` (cash+in-kind) → `equityCashArr` only. Net BS Cash unchanged; both lines individually right. New `ProjectBS.statutoryReserveTransferPerPeriod` + `dividendsPerPeriod` (placeholder).

**Pass 2Q — IDC integrated into Fixed Assets roll-forward + Land+BUA sqm on IDC summary.** `AssetIDCRow` gains `physicalLandSqm` + `physicalBuaSqm` (always populated, independent of active basis); IDC table shows both sqm + share %. Module 4 Fixed Assets Depreciable Roll-Forward integrates IDC: `Opening (Capex + IDC) + Capex Additions + IDC Additions − Depreciation (on Capex + IDC) = Closing (Capex + IDC)`. Module4FixedAssets calls `computeIdcSnapshot` via the standalone helper.

**Pass 2R — Funding Gap sub-tab (Method 2 + Method 3 simple).** New `computeFundingGap(snap)` helper + `FundingGapSnapshot` type. Sub-tab in Module 1 Financing with Method 2 (Capex vs Pre-Sales) + Method 3 (Pre-Financing CF Deficit simple form). Verifier M-series pins both methods' identities.

**Pass 2R-Fix — Sell capex spread + E1 prior col + CF subtotal priors.** Sell asset Inventory build-up switched from uniform spread → `computeAssetCost().breakdown.perPeriod` projected onto axis (same offset rule as fixed-assets-resolvers). E1 Equity Schedule: existing equity carry-forward moved from Y0 axis lump to the prior-year column. CF Direct subtotals (Total Capex / CF Investment / CF Financing / Net CF / Opening cash / Closing cash) carry `priorValue` from component rows. BS Check warning enhanced with top-5 imbalance years.

**Pass 2S — Cash Sweep debt-repayment function + Method A reshape + BS IDC merge.** `FinancingTranche.cashSweepConfig` extended with `enabled` + `priority` + `startingYear` (default = construction end + 1) + `sweepRatio` %. New `computeCashSweep` helper (forward-pass, V1 doesn't re-derive future interest). Composer wires sweep into BS Cash + BS Debt + Direct CF. UI: per-tranche editor (priority/start year/sweep ratio) + new "Cash Sweep — Debt Repayment Priority" panel in Funding Gap. Method A reshape per user's reference (Capex / Pre-Sales gross / Less escrow held / Add escrow release / Pre-Sales net / Fulfilled / Gap MAX(0, …)). BS: merged "Capitalised IDC NBV" line into "Fixed Assets (NBV, incl. capitalised IDC)".

**Pass 2T — Dividend waterfall (before-sweep → cash sweep → after-sweep) + EBITDA cap.** `Phase.dividendPolicy = { enabled, priority: 'before_sweep' | 'after_sweep', startingYear, payoutRatio }`. Defaults: operational phases → before-sweep, new phases → after-sweep. Engine refactored `computeCashSweep` → `computeCashWaterfall` returning both `CashSweepSnapshot` + `DividendSnapshot`. Composer wires dividends into `bs.dividendsPerPeriod` + RE + Direct CF + closingCash. Pass 2T-Fix: cumulative dividend per phase ≤ cumulative phase EBITDA; Method A pre-sales lag 1 period (`gap[t] = MAX(0, capex[t] − preSalesNet[t-1])`). Pass 2T-Fix #2: Method B same one-period ops lag.

**Pass 2U — Method 3 detailed waterfall + rename Method A/B → 2/3.** Method 3 rebuilt as detailed per-period waterfall (Opening → CFO → CFI → existing financing items − Phase 1 div = Cash Available − Min Cash = Cash for Debt+Div Payment − Sweep − after-sweep div = Closing). Renamed Method A → "Method 2 — Net Funding Requirement", Method B → "Method 3 — Cash Deficit Funding" aligned with Funding Method dropdown. Pass 2U-Fix: dividend priority auto-assigned by phase status (removed user toggle), per-phase `cashAvailableForDividendPerPeriod` exposed, Method 3 double-count fix (existing equity/debt removed from t=0 inflows since already in opening cash). Pass 2U-Fix #2: prior column added to all Financing tables (SchedulesView + FundingGapView via Pass 2V); Method 3 splits Net Cash Required into debt + equity per funding ratio + Total New Debt (incl IDC) + Total New Equity grand totals.

**Pass 2V — Consolidated Cash Sweep + Dividend master waterfall + prior column on Inputs tables.** Replaced 3 separate Method 3 + Cash Sweep + Dividend Schedule tables with ONE master "Cash Sweep & Dividend Waterfall" matching user's reference model. Inputs sub-tab tables (Capex Breakdown / Funding Requirement / Total Debt Required / Total Equity Required) gain prior-year column with pre-axis items routed there.

**Pass 2W — Phase 1 i=0 lump rescue (BS imbalance root cause fix).** The asymmetric projection rule between capex engine (dropping Phase 1's Y0 lump at `projIdx = -1`) and equity engine (stamping in-kind at `axis[0]` regardless of phase) caused a ~1.4M BS imbalance during construction. Fix: 3 sites updated to `projIdx = i === 0 ? Math.max(0, offset - 1) : offset + i - 1` so Phase 1's Y0 lump lands at axis Y0. Phase 2+ unchanged.

**Pass 2X — D-4 existing-debt double-count guard + D-1 FA prior column.** `buildExistingAggregate` now skips existing tranches whose `originationYear >= projectStartYear` from `debtOutstandingTotal` (those openingBalances are already drawn via `drawSchedule[origIdx]` as in-axis inflows — counting them on the prior column too was double-counting). Module4FixedAssets PeriodTable gains optional `priorYearLabel` + `Row.priorValue` so opening Land + opening NBV render in the prior column.

**Pass 2Y — D-3 Cash Sweep interest savings memo.** `CashSweepSnapshot` exposes `interestSavingsPerPeriod` (per-tranche balance reduction × periodic rate) + `totalInterestSavings`. Rendered as memo row "(memo) Interest Savings (informational; P&L still uses pre-sweep balance)" in the master waterfall. Full P&L mutation deferred to a future iterative pass (BS still balances under sweep without it).

**Pass 2Z — IDC column alignment fix + per-parcel in-kind stamping.** (1) IDC Allocation table inline `<tr>` rows missed the empty prior-column cell post Pass 2V → axis Y0 values rendered in Dec 25 (prior) column. Fixed by inserting empty prior cell in all three inline tr blocks (per-asset summary + sell routing + op/lease routing). (2) `debtEquity.ts` previously stamped ENTIRE in-kind sum at `axis[0]` regardless of parcel's phase. After Pass 2W, Phase 1+Phase 2 were balanced but Phase 3+ (offset >= 2) still had Y0 equity with no matching Land asset until offset-1. Now `computeDebtEquitySplit` accepts `phases` + `project` and stamps in-kind per parcel at `projIdx = Math.max(0, offset - 1)` of the owning phase — symmetric with the asset side. Signature: `computeDebtEquitySplit(capex, funding, parcels, parcelFunding, axis, phases?, project?)`.

**Architectural conventions added this session** (apply to all future modules):
...
3. **Manual reconciliation** of the BS Build against reference v1.16 before further automation.
