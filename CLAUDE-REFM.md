...
**Last updated: 2026-05-19. Module 1 LOCKED at M2.0 Pass 58. Module 2 (Revenue + CoS + Schedules) LOCKED at Pass 9g-O. Module 3 (Opex) WIP on Pass 3. Verifiers: revenue 133/133; opex 32/32.**

**Module 3 status (Opex, Pass 3 complete):**
...
- **Resolver** at `src/hubs/modeling/platforms/refm/lib/opex-resolvers.ts`: `computeAllOpexResults(state, revenueSnap)` returns `ProjectOpexSnapshot` (byAsset map + projectTotals + hq result + totalOpexPerPeriodInclHQ). Default-seeds lines when an asset / project has none and threads asset / HQ `defaultIndexation` to the engines.
- **Schema**: `Asset.opex.defaultIndexation` + `Asset.opex.lines[]` per-asset, `Project.hqOpex.defaultIndexation` + `Project.hqOpex.lines[]` project-wide. Same shape as the engine `OpexLine` + `IndexationConfig`. All fields optional — defaults seed on first read.
- **UI** (Pass 3 rewrite): `Module3Opex.tsx` renders a top-level **Asset Inflation** panel inside every asset card and the HQ card (4-pill Off / Flat / Compound / Per-Year + rate input + per-year strip). The line table's Inflation column now shows `— auto via revenue` for %-of-rev / pct_of_gop modes and an **Inherits: <method> <rate>** badge with an **Override** button for fixed-cost lines. Click Override and the row expands a compact `InflationPanel` for that line; **Use asset default** flips back. **Apply to all Hospitality / Retail** copies BOTH `defaultIndexation` and `lines[]` to every other matching-strategy asset.
- **Verifier** `scripts/verify-opex.ts` 32 / 32: A/B/C/D/E unchanged (existing inflation behaviours preserved via `defaultIndexation: defaultOpexIndexation()` passed in inputs); F-series pins the Pass 3 rules: F1 %-of-rev ignores line indexation, F2 pct_of_gop ignores line indexation, F3 fixed line inherits asset default, F4 per-line override beats default, F5 HQ fixed inherits HQ default, F6 HQ pct_of_total_rev never indexes.
