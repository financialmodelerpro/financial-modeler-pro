...
**Last updated: 2026-05-13. Current phase: M2.0 Passes 14-18 (Tab 4 Final Redesign + Verification Fixes), 29 commits total since Pass 13.**

**M2.0 Pass 18 themes (2026-05-13, Tab 4 Verification Fixes, 9 commits):**
- **Land Funding card collapsed to Debt% / Equity% inputs (commit `996f1b1`).** Per-parcel dropdown (5 options) + custom-split editor + deferred schedule editor → single auto-paired Debt% / Equity% pair (sum = 100). New `ParcelFundingConfig.debtPct? / equityPct?` fields (additive); `fundingType` + `customDebtPct` + `customEquityPct` + `deferredSchedule` marked `@deprecated` (retained for back-compat). Migration `migrateM20pass16LandFundingSimplify` maps legacy enums: `100pct_equity` / `in_kind` / `deferred_payment` → `{0, 100}`; `100pct_debt` → `{100, 0}`; `custom_split` → `{customDebtPct, 100 - customDebtPct}` (normalised to reconcile per Fix 8 audit). `parcelDebtEquityFractions` resolver prefers new fields; falls back to legacy enum path.
- **Capital Structure Overview removed (commit `d7f07fe`).** Entire 7-card block + per-facility breakdown `<details>` deleted. Sources vs Uses match check inlined as a small chip next to Total Capex on the Funding Basis row (green ✓ Match / amber Gap: {currency}). Tooltip carries the exact sources/uses/gap numbers. All previously surfaced values (Total Funding, Total Debt, Equity Cash, Equity In-Kind, Debt %) still visible in downstream Funding Requirement + Debt Required + Equity Required tables.
- **Funding methods renumbered to 1 / 2 / 3 (commit `9674bb1`).** `FundingMethodId: 1|3|4 → 1|2|3`. Method 1 = Fixed Debt-to-Equity Ratio (unchanged), Method 2 = Net Funding Requirement (was Method 3), Method 3 = Cash Deficit Funding (was Method 4). Migration `migrateM20pass17MethodRenumber` flips legacy `fundingMethod` values. Labels + descriptions + engine branches + UI conditional checks + verifier scripts all updated. `m3-*` data-testids → `m2-*`, `m4-*` → `m3-*`.
- **Engine: Methods 2 and 3 affect non-land capex only; land always routed by parcel (commit `dd52bd8`).** `computeFunding`'s land/non-land split now applies to ALL three methods (was Method 1 only). For each method: split capex into `nonLandCapex` + `landCashPerPeriod`; apply method-specific sizing (fixed-ratio / net-funding / cash-deficit walk) to non-land; route land cash uniformly via `parcelDebtEquityFractions`. Method 1 + 4 / 2 / 3 ratios applied to non-land debt/equity split; land debt/equity derived independently per parcel. Fallback path preserved when callers omit `landCashPerPeriod`.
- **Capital Stack Summary block dropped from Schedules (commit `2199cdd`).** Schedules sub-tab block 1 removed; remaining 5 blocks renumbered: Debt Movement (1), Combined Debt Service (2), Finance Cost (3), IDC Summary (4), Equity Movement (5).
- **Schedules drawdown wired from Funding Requirement (commit `d729c02`).** `computeFinancing` gains optional `precomputedDrawSchedule?: number[]` arg; when supplied, skips the legacy `tranche.drawdownMethod` switch and uses the supplied series verbatim. `resultsMap` moved BELOW the `funding` memo and now derives per-facility drawdown = `funding.debtEquitySplit.debt[offset+i] × facilitySharePct / 100` (the only allocation rule today; no waterfall). Existing facilities (`origin === 'existing'`) skip the arg and amortise from `openingBalance`. `tranche.ltvPct` + `drawdownMethod` marked `@deprecated` (engine ignores them when `precomputedDrawSchedule` is supplied; UI no longer exposes them).
- **YoY% editor uses repayment periods (commit `e2594b8`).** Editor period count = `tranche.repaymentPeriods` (new) or `remainingRepaymentPeriods` (existing); when neither is set, falls back to `phase.constructionPeriods`. `makeDefaultFinancingTranche.repaymentPeriods` default lowered from 60 to 0 so newly-instantiated tranches don't render a P1-P60 grid (UI's `handleAddTranche` still sets `phase.operationsPeriods` on creation).
- **Percentage reconciliation audit (commit `e2a6cb4`).** Land Funding render normalises equity display to `100 - debt` so on-screen pair always reconciles; Pass 16 migration normalises legacy custom_split pairs to sum 100. m1/m2/m3 debt/equity inputs auto-pair on change. YoY sum chip already uses `.toFixed(2)%`.
- **Methods 2 + 3 blank until M2/M4 ship (commit `3e918ef`).** Funding Requirement table `renderRow` adds `isMethodStubbed(m)` guard returning `true` for methods 2 + 3. Stubbed rows render `-` for Total + every period cell; Selected row also renders dashes when active method is 2 or 3. Engine still computes them (no engine change); only display blanked. Flip the guard to `false` when M2 Revenue + M4 FS engines wire.

**M2.0 Pass 15 themes (2026-05-13, Tab 4 Final Redesign Pass, 9 commits including 5b polish):**
- **Period axis matches Tab 3 + 1 trailing year (commits `d732cb6`, `3a5b767`).** New `inputsAxis` memo derives `first`/`last` non-zero indices from `inputsSummary.totals` (with Y0 anchor when `projectInKindLandValue > 0` to match Tab 3's inclusive-extent start), then `numAnnualPeriods = last - first + 1 + 1` (the +1 trailing year). Capex Breakdown + Debt/Equity Required + Funding Requirement tables all consume `inputsAxis.axis` and crop their row data via `inputsAxis.cropRow(...)`. Dead full-duration `labels` + `generatePeriodLabels` import dropped from `inputsSummary`. Schedules sub-tab axis untouched.
- **Tab 4 Inputs layout reordered (commit `c2c488d`).** Capex Breakdown moved from position 2 to position 7. New top-to-bottom: Project Financing Settings → Funding Method → Funding Basis → Land Funding → Capital Structure Overview → Debt Facilities → Capex Breakdown → Funding Requirement → Total Debt Required → Total Equity Required.
- **Input sections compressed (commit `441e189`).** Project Financing Settings tightened to single flex row (label + input + helper inline). Funding Method changed to horizontal 3-card grid (`repeat(3, 1fr)`). Funding Basis dropped redundant Method field (3 fields: Drawdown Basis / Total Capex / Total Funding Need). Capital Structure Overview compressed from 7 cards to 1 headline + 6 compact cards (later removed entirely in Pass 18).
- **LTV wording removed across Tab 4 (commits `b6a7a30`, `0faf40b`).** Tranche covenant label → "Max Debt %"; Capital Structure card → "Debt %" (later removed). All comment + helper text rewordings. `DRAWDOWN_METHOD_LABELS.capex_basis` and types comments updated. Internal schema field names (`ltvCovenant`, `stack.ltvSenior`, `stack.ltvTotal`) + data-testids retained.
- **Funding Requirement table (commit `e4d15c5`).** New IIFE block between Capex Breakdown and Total Debt Required. Calls `computeFunding` 3 times per render (one per method); 4 rows rendered: Method 1 / Method 2 / Method 3 / Selected (mirrors active method via `ROW_GRAND_TOTAL` styling). Footnote about M3/M4 stubs (later flipped to dashes in Pass 18 Fix 9).
- **Per-facility Grace Interest Treatment (commit `74143d5`).** New `FinancingTranche.graceInterestTreatment: 'pay_from_ocf' | 'add_to_funding_need' | 'capitalize'` (additive). Migration `migrateM20pass15GraceTreatment` backfills `'capitalize'` on legacy tranches; new tranches default to `'pay_from_ocf'`. UI dropdown next to Grace Period in TrancheCard (visible for both new + existing facilities). Engine wire: when method = 2 (Net Funding) and treatment = `'add_to_funding_need'`, accrued grace interest = `principal × rate × graceYears` distributed evenly across grace periods, added to `capexPerPeriod` for the Method 2 funding computation. `m3GraceCapexAdd` memo shared between the `funding` memo (when method=2) and the Funding Requirement table's M2 row.
- **Tab 1 per-asset pre-capex with debt/equity validation chip (commit `839e066`).** New `Asset.historicalPreCapex / historicalDebtAmount / historicalEquityAmount` fields (additive). Operational-phase reveal in `Module1ProjectPhases.tsx` adds a "Per-asset Historical Baseline" sub-panel: 5-col grid per asset (Name / Pre-Capex / Existing Debt / Existing Equity / validation chip). Chip is green "Balances" when `pre = debt + equity` (within rounding) or amber "Mismatch: equity should be X to balance" with diff tooltip. New `historicalPriorTotals` memo on Tab 4 sums per-asset values across operational-phase visible assets; Capex Breakdown prior cell gets `preCapex`, Total Debt Required prior gets `debt`, Total Equity Required prior (cash + grand-total) gets `equity`. Existing `Phase.historicalBaseline` retained for back-compat.

**M2.0 Pass 14 themes (2026-05-13, Universal Annual Basis, 3 commits):**
- **Annual-only basis until M5 FS (commit `5701b19`).** Granularity toggle removed from REFM platform (Tab 3 Results + Tab 4 Inputs + ProjectWizard). Shared `_shared/GranularityRadioBar.tsx` deleted. `project.outputGranularity` marked `@deprecated`. Tab 3 Costs Results `transformAnnualSeries` reduced to identity (quarterly + monthly distribution branches deleted). M5 Financial Statements will reintroduce granularity scoped to FS output only.
- **Data-driven period axis (commit `db8596e`).** 60-year hard cap removed everywhere. `buildResultsPeriodAxis` no longer takes a `granularity` arg or applies a cap; caller picks `numAnnualPeriods` from project duration + active-data extent. Tab 3 Results computes `annualPeriodCount = max(totalConstructionPeriods, activeLastAnnual + 1, 1)`. Tab 4 Schedules computes `periodCount = max(inputsSummary.totalPeriods, combined.periods, 1)`.
- **Column widths re-balance to 22% / equal-others (commit `18084dd`).** `COLUMN_WIDTHS = { label: '22%' }` + `nonLabelColumnPct(count)` helper returning `(78 / count).toFixed(4)%`. Every results table renders `<table style={{ width: '100%', tableLayout: 'fixed' }}>` with `<colgroup>` of label@22% + N × equal-pct cols. Extending a phase's operating periods rebalances every non-label column proportionally; label stays at 22%.
