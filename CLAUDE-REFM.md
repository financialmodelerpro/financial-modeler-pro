...
- **Prior-year column convention**: every financing-related table renders a prior column (axis.priorLabel = projectStartYear − 1) between Total and the first axis year. Pre-axis events (existing equity, existing debt opening, pre-capex, opening NBV / Land) appear in the prior column; axis values shift right by one cell. Inline `<tr>` rendering MUST include the empty prior cell to align with `renderFlowRow` / `renderStateRow` headers.
