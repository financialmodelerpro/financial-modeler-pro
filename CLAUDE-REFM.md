...
## 2026-09-24: the Activity log reads as what people did

The append-only log (`refm_project_changes`) is made readable at READ, by the rules the writer uses, so every stored row benefits, not only future ones (808 rows on Marina Gate, 0 of them labelled). Files: `lib/persistence/changeLabel.ts` (`labelForChange`, the ONE sentence rule, which the differ now calls for every entry; `SCREEN_WORDS`, each field in the words of the screen that writes it; `fieldRole` edit / computed / internal; `presentChanges`, one edit per row with everything left out counted by reason; `describeBulk`), `lib/persistence/valueText.ts` (`formatValue` / `describeChange`: values by their field through the screens' own label maps, references as names, "not set", a changed list said once). Surfaces: the Activity panel, the Version modal's change list and the case switcher, all through `useModelNamingContext`. A bulk row now records every path. A case override carries "Edited on ..., with that case active". A legacy save is recovered by timing ONLY to fold consequences (founder-approved; display grouping is still by recorded save id). Probe: `scripts/probe-change-log-readability.ts` (read only). Full story in [CHANGELOG.md](CHANGELOG.md) 2026-09-24; lesson TRAPS 7.57; `verify-change-log` 185.
...
**Three passes, each deployed same day with the health SHA matched to HEAD. Full dated narrative in [CHANGELOG.md](CHANGELOG.md) 2026-08-30j/m/n/o; the standing lesson in [docs/TRAPS.md](docs/TRAPS.md) 7.31.**

**(1) COST OF SALES HAD TWO ENGINES (`d3d43445`).** The P&L used `buildCostOfSales`; the Module 2 screen and both exports used `buildCostOfSalesV2` on a base each surface re-assembled by hand. The reported premise ("they agree on lifetime totals") was **wrong and worth correcting**: they agreed to 0.24% on RE HUB, which is what hid it, and were **25.4% apart over the lifetime on Marina Gate**; per period they were **407,131,731 apart in one year** (34.7% of the P&L figure).

**The decision was the deliverable, not the reconciliation.** v1 won because `SellAssetResult.recognitionPerPeriod` IS the series the P&L books as revenue, so spreading the base across it is matching in the accounting sense and holds **gross margin constant per asset** (Marina Gate 60.5% every year). v2's factor was a product of cumulatives rather than the revenue engine's own `recognitionVintageMatrix`, and swung the margin **75.7% -> 66.0% -> 43.4% -> 6.4% -> 27.6%** on one product. A 6.4% gross-margin year between a 43% and a 28% year is an artefact, not an economic pattern.

**Two base defects came with the loser and are fixed at source**, not reproduced: the hand-rolled Y0 rule (`i === 0 ? offset - 1 : ...`, which is -1 for a phase starting with the project) **dropped 56,375,000 of capex off the axis**; and `computeAssetCost` truncated any line distributed past its sized window (`Math.min(dist.length, periodSlots)`), so **`.total` stopped equalling the sum of the periods beside it** (6,561,759 on a Marketing line, 10,632,560 on a Commission line, both selling costs following collections past the construction window).

**What actually made it findable was a NEGATIVE CLOSING INVENTORY.** The balance sheet builds inventory as cumulative capex less cumulative cost of sales, so Marina Gate closed at exactly **-6,561,759** and RE HUB at exactly **-10,632,560**: the truncated amounts, showing up as the visible end of a silent subtraction. A roll-forward that must close at zero is a free reconciliation of two independent series, and it beat every verifier in the suite to this.

`lib/costOfSales.ts` is now THE Module 2 cost-of-sales layer (base + IDC + spread + inventory + vintage, assembled once), read by the P&L, the balance sheet, the screen, both PDFs and the workbook. **Measured after: cost of sales ties to the P&L in EVERY period on both projects (max gap 0.00), the Module 2 roll-forward IS the balance-sheet series, and both close inventory at 0.** The consolidation itself moved **no number anywhere**; every movement traced to the truncation fix and its knock-on through capex -> debt -> IDC (Marina Gate FCFE IRR 37.75 -> 37.17%, RE HUB 8.04 -> 8.02%).

**(2) THE PORTFOLIO DASHBOARD SHOWS PORTFOLIO MONEY (`2026-08-30j`, then cleaned up in `e5c3cafd`).** Three of its five tiles counted a workflow status nothing in the product ever sets, so they were structurally zero rather than stale. Replaced with engine-derived money on the Management base case, with the rules that make a portfolio number honest rather than plausible: **IRR computed ONCE over the summed streams** (8.91% against a 22.85% naive average of 37.7% and 8.0%), **peak debt as the max of the SUMMED series** (2,873.3m against a 2,938.2m sum of per-project peaks), a **dense calendar axis**, per-currency totals, and un-modelled projects counted out. **The verifier caught a real bug in my own aggregator**: a sparse axis discounted a gap-year return as if it arrived earlier, reading **9.50% against a true 4.87%**.

**(3) THE FUNDING CHART BELONGS TO THE PROJECT (`13661f36`).** Removed from the portfolio (tiles only); the dependency question split three ways and was checked before deleting anything: `debtByYear` and `fcfeByYear` are load-bearing (peak debt, portfolio IRR) and stayed; `fundingByYear` fed the chart alone and went with it. The single-project chart now sits at the foot of Overview's Cost & capital structure, where the Peak Equity tile raises the question of when capital is needed, with amounts printed on the bars. The trim rule lives once in `lib/portfolio/fundingSeries.ts`: empty head and tail dropped, **interior gaps KEPT**, because closing one up re-introduces exactly the axis compression that overstated the IRR.

---

## 2026-08-20h: watermarked trial exports, honest export gates, zero-seed OpEx, and a tour that never spotlights a blank

**Five platform passes after 20g, every one deployed same day with health SHA matched to HEAD. Narrative compact; the commits carry the detail.**

**(1) TRIAL PDF WATERMARK + THE POWERPOINT GATE (`9c7f8a88`).** One pure rule (`src/shared/entitlements/exportWatermark.ts`, plan list, default ON for trial only) that the admin card on /admin/plans and the exports BOTH read; the server resolves the spec (`/api/export/watermark`, no client input) and the browser can only obey or refuse, with a failed resolution REFUSING the export rather than emitting a clean file. `applyExportWatermark` stamps every page of the full report, the summary report and the IC deck PDF over the finished document (a new page type is covered the day it is written); a paid export is BYTE-IDENTICAL, proven on real PDFs. `white_label_pdf` deliberately untouched: two flags, two jobs. **The deck route had NO per-plan check**: an active trial reaching the endpoint got an editable .pptx, kept out only by the Module 7 editor being unreachable in the trial UI (a gate made of a screen). Now 403 `FEATURE_NOT_INCLUDED` on the shared `moduleFeatureKey('reports', 7)`, after ownership, before anything builds, both formats. `verify-export-watermark` 64.

**(2) EXCEL GATES ON `excel_snapshot` (`2d74ff3e`).** It gated on `excel_formula`, `included:false` on ALL FOUR plans, so pro and firm were denied an export they are sold; the workbook writes constants, so the snapshot key is the honest one. Trial/solo stay denied; no plan carried the old key, so the swap can only open what was wrongly shut, confirmed against the LIVE `plan_permissions`. Nine picker-card call sites had respelled the keys inline; all now read `FEATURE_FOR_KIND`, the only place a key is named. `verify-export-watermark` 64 -> 84 (live-permissions section H).

**(3) OPEX SEEDS ZERO (`9c7f8a88`).** All 26 seeded values across the HQ / hospitality / lease builders (HQ payroll 5,000,000, F&B 65% of revenue...) are now 0; every line item stays. The ENGINE fell back to the builders too, so a new project silently carried 109.065m of HQ opex and 111.656m less PAT. A disabled line still HOLDING a seeded value is cleared on hydrate (`clearSeededDisabledOpexValues`, matched on category AND mode AND exact figure so a user's own parked number survives; 27 lines on MARINA GATE, 3 on RE HUB, and NO NUMBER MOVES because every touched line is disabled, proven per project). `LEGACY_OPEX_SEED_VALUES` is a FROZEN record nothing may seed from. RE HUB's live enabled `Hotel 01 / F&B direct cost` still sits at the seeded 65% and was deliberately left: it is charging today and is the user's question, not ours. `verify-opex-seed-zero` 45 NEW; `verify-opex` now states its own fixture inputs from the frozen record (its E-series had really been testing the DEFAULTS).

**(4) THE TOUR NEVER SPOTLIGHTS A BLANK (`0636d130`).** Two modes from one builder: no project open -> module-level walk (welcome + 7 modules + finish, 9 steps); project open -> the full 34. Within the full walk each tab step reads the ACTUAL RENDERED DOM before showing anything (`surfaceIsEmpty`: the platform's own `-no-project` markers, else substance = fewer than 3 table/input/select/textarea AND under 160 chars of text; two reads 200ms apart must agree; the skip travels in the user's direction; no card until the spotlight resolves). NEVER a tab list of "usually empty" tabs, and the verifier forbids tab names inside the detector. `completedMode` records which shape finished; the guide restart consults no completion state, so a module-only run never blocks the full tour. **First-run guide prompt** before the tour offer, once per user, both actions stamp `guidePromptAt` in the SAME mig-217 blob (which forced MERGING writes in `tourState`, TRAPS 7.30); re-showable from the guide beside the tour restart. `verify-platform-tour` 145 -> 180 (incl. the rendered-text dash ban, TRAPS 11 family).

**(5) TOPBAR (`09054fb8`, `c3c413e2`).** The ADMIN role badge, tooltip and RbacModal mount are PARKED for Module 8 (role LOGIC untouched: `currentUserRole` / `can()` / `ROLE_META` still gate everything; `RbacModal.tsx` stays on disk unimported; the sidebar footer indicator, the one other reader, is now display-only because a click that opens nothing is a lying control, TRAPS 7.20). Bare `.pm-btn` carries no colour, so Guide, then Save-as-new-version, then Colours all rendered black on the navy bar; all three now use the `--color-on-primary-navy` token trio, and `verify-platform-guide` sweeps EVERY bare pm-btn in the Topbar as a CLASS so the next one fails the suite instead of being reported from the screen (150).

**Verifier discipline bought twice today (TRAPS 10.11/10.12):** a surface hidden behind `false &&` greps as present, so render CONDITIONS are pinned beside markup; and a check whose regex was eaten in a patch pipeline failed CORRECT code and was committed red once (`9cf2994b`, fixed `be584e58`), so the commit gate is a green run on restored code as the LAST command.

## 2026-08-20g: the guide is rebuilt against the platform, and a guided tour derives from it

**GUIDE (commit 7dbf8409).** The old guide was a 760px dialog with one long scroll, no nav and no search, and its content stopped at Module 5: Modules 6 and 7 rendered as their bare names, Parties and Fund Terms were label-only, M2 still described the retired payment profile, M1 listed four cost stages, M5 two terminal methods. verify-platform-guide passed 34/34 throughout because it checked a FROZEN COPY of the tab map (TRAPS: mirrored lists diverge). Three structural fixes: **(1) ONE TAB REGISTRY**, m1Tabs..m7Tabs + MODULE_TABS moved out of RealEstatePlatform.tsx (CSS import, not loadable under tsx, the reason the copy existed) into pure lib/moduleTabs.ts; the shell re-exports, Sidebar untouched, the verifier imports the same module the shell renders. module6 is a DELIBERATE empty tab list (one page, four declared surfaces with real m6-* anchors). **(2) ONE CONTENT SOURCE**, lib/guide/guideContent.ts, rewritten current (sale cohort terms + grid + hard cut-off + project default, five stages, catalog identity, positional % of selected, derived phasing sources, shares-as-typed, country select, zero-seed OpEx contract, Exit Cap Rate, FCFF unlevered full cost, fund layer, M6/M7 in full). platformGuide.ts only assembles; Markdown + PDF serialise the same GuideDoc. **(3) FULL-SCREEN OVERLAY** (PlatformGuideModal.tsx rebuilt): side nav to every section/module/tab, live search with empty state, Escape closes, no backdrop click-away, tokens only (verifier fails on any literal colour). verify-platform-guide 34 -> **143**, four landing-confirmed sabotages; the key one: A NEW TAB WITH NO CONTENT NOW FAILS THE SUITE.

**TOUR (commit c266a99a, mig 217 PENDING apply).** 34 steps: welcome, every module in canonical order, every tab within it (M6 walks its four surfaces), finish pointing at the Guide button. **Step text IS guide text, by string identity** (module steps render MODULE_INTRO verbatim, tab steps the tab intro; the verifier asserts equality, and the builder carries exactly two sentences of its own). Navigation reuses fmp:trace-to; the spotlight is a box-shadow cutout with a fallback anchor chain and retry, degrading to a centred card. Skip / pause / back / resume; Escape pauses; every exit and step change persists. **Per-user state**: users.refm_tour jsonb (mig 217, additive) behind /api/refm/tour-state, schema tolerant (available:false -> localStorage fallback), auto-runs once per user and never after completed/skipped, restartable from inside the guide. NOTE: EVERY existing user gets the tour once on their next platform open, one Skip retires it for good. verify-platform-tour **145 NEW**, four landing-confirmed sabotages. Neither pass touches an engine path; no number moves.
