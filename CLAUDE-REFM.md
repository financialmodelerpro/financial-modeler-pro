...
**Next session pointer (2026-05-19 EOD):** Ahmad will verify Module 3 Pass 3 + Pass 4 in-browser (asset-level Inflation panel, per-line Single / YoY rate-mode toggle, category-wise Output tables, retail-lite seed, universal sidebar dropdown). Tweaks land first; then we move to **Module 4 Financial Statements** which composes the P&L (Revenue − CoS − Opex − D&A = NI), Balance Sheet (NWC + D&A + retained earnings), and Cash Flow (Direct + Indirect) from the M2 schedules feed + M3 opex snapshot. No code changes for M4 yet — open with a scope-pass MD that mirrors the M2 / M3 narrative shape (engine first under `src/core/calculations/statements/`, resolver bridging M1+M2+M3, then per-statement UI surface).
