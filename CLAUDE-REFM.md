...
- **UI**: `Module3Opex.tsx` (Inputs) + `Module3OpexOutput.tsx` (read-only narrative). Inputs is a per-asset line-item editor with mode-aware value cell (% or accounting), 3-button segmented Inflation control (commit `062c168`, 2026-05-18: always-visible Off / Flat / Yearly pills; Yearly auto-expands a per-year % strip on the row below; Yearly disabled on HQ since HQ has no asset ops window), on/off checkbox, remove + add-line. **Apply to all Hospitality / Retail** button per asset card (commit `faf16c8`): clones current asset's lines onto every other matching-strategy asset with fresh per-asset line ids. Output renders per-asset Drivers → Direct → GOP → Indirect → Mgmt → Other → NOI plus HQ + Project Total tables.
