...
## 2026-06-16 session: M6 scenario audit + admin module mgmt (commits 0b7f7fe0, 1068dfc3, 246105de)

**Group A, dead-override audit + fix (`0b7f7fe0`).** Audited every Module 6 grid override end-to-end. Only genuine dead override = opex inflation: a grid write sets `defaultIndexation.rate` with NO `method`, which the asset/HQ resolver discarded (gated on `stored.method`) and `applyIndexation` no-ops without a method. Fix = `normalizeOpexIndexation()` (`core/calculations/opex/defaults.ts`): a rate-only config is coerced to `yoy_compound`; a method-bearing config is returned unchanged (base results never shift, since method-less-but-rated only arises from an override). Applied in `opex-resolvers.ts` (asset + HQ) + mirrored in `Module3Opex.tsx`. NOTE the asset lever is small under the default seed (only the 2 fixed-cost lines index, %-of-revenue lines never escalate, by design); HQ opex inflation is the large lever. `verify-module6-scenarios` 127/127.

**Group B, grid polish + comparison (`1068dfc3`).** `assumptionGrid.ts`: Tax/Zakat dropped from curated (constant across cases, still under Show all); construction levers relabelled to mirror the Capex line names + units (`Construction (BUA), per sqm` / `Construction (Parking), per bay`); **land purchase price** surfaced as a curated `parcels[id=X].rate` lever under Construction & Capex (round-trips natively, land cost lines are locked/derived from parcels). `Module6Scenarios.tsx`: assumptions grid is a bounded scroll area (`maxHeight 70vh`) with `position: sticky; top` header. `caseComparisonReport.ts` CASE_KPIS: added Total Financing Cost / Cap Rate at Exit / Min DSCR, and split Total Development Cost into Land Cost + Capex (construction) (= TDC). All read existing snapshot fields (`developmentEconomics.totalFinancingCost`, `realEstate.capRateAtExit/dscrMin`, `sourcesUses.land/construction`).

**Group D, project-open model + Dashboard/Overview split + sidebar (`57cac5f5`).** No default project: the boot effect no longer auto-opens the cached `readActiveProjectId`; sign-in lands on the Dashboard with NO project open. Universal module lock: `Sidebar.tsx` disables Overview + every `module1..11` when `!activeProjectId` ("Open a project first"); Dashboard / Projects stay accessible (they are not module keys). Dashboard vs Overview de-duplicated: `renderModule` previously routed BOTH `dashboard` and `overview` to the same project-scoped `Dashboard`. Now `Dashboard.tsx` is rebuilt as the project-AGNOSTIC all-projects hub (portfolio stat tiles, recent activity, projects list + empty state, reads `storage.projects` only), and a NEW `Overview.tsx` is the open-project investor summary (headline returns / key economics / cost split / capital structure / timeline / health chips) reading `computeFinancialsSnapshot` -> `computeReturnsSnapshot` fields only (no engine change; no module-nav cards). Sidebar width 304->350 (a further 15%; the prior change was on the correct `.sidebar` element, the wrapper's `zoom: 0.8` just shrank the visual delta). Build 213/213, verify-psync 70/70, verify-module6 127/127.

**Group C, admin module reorder/hide + sidebar (`246105de`).** Root cause: `number` was conflated as display number AND routing identity (`module${number}`). Fix WITHOUT migration (display_order is unconstrained): `number` stays the stable routing identity, `display_order` drives the sequence, the DISPLAYED number is the 1-based display_order position. Admin (`app/admin/platform-modules/page.tsx`) gets ▲/▼ arrows → `moveModule` rewrites `display_order = position` + persists. `usePlatformModules.ts` sorts by display_order, labels by position, fetches `cache: 'no-store'` (admin changes reflect on next mount). `RealEstatePlatform.renderModule` guards any `moduleN` key absent from the visible list as non-routable (hidden = gone from sidebar AND unreachable). Sidebar width 264→304 (`--sidebar-width`, +15%). Marketing (`app/modeling-hub/[platformSlug]/page.tsx` + `[moduleSlug]/page.tsx`) already read the same DB config (ordered by display_order, hidden filtered); switched their displayed MODULE number from identity to position so a reordered/skipped module reads consistently everywhere. `verify-psync` 70/70.
