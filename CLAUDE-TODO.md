...
0h. **OPEN, A SCENARIO CANNOT VARY A PRICE AT TYPE LEVEL (logged 2026-09-15).** The price per unit and per sqm
   per type on Types and Standards reach the model only through the store, which writes them onto each Table 5 row with
   no price of its own. A scenario override is value-only and never re-runs that step, so Module 6 shows the two type
   ...
   pipeline running the same settle (type prices onto unstated rows) before the engine, which would make the two
...
0f. **FIXED 2026-09-15 (was reported earlier the same day):** (1) Land 2 has no `assetTypeId` but its free-text
   ...
   `resolveAssetTypeKey` in both. (2) The Standards tab's explanatory paragraph still says the type names "wait for
   Save"; the names have saved as you type since 2026-09-10, and the only explicit save on the tab is "Save this list to
   the firm's template". **CLICK-TEST OWED:** the soft cost basis selector and the Capex picker's basis note.
...
## SUPERSEDED 2026-09-12 (still current where noted): START HERE 2026-09-09: MODULE 1 LAND PLANNING + CONSOLIDATION STEPS 1-5 ARE LIVE. WHAT IS OPEN, in order:
...
### 8. THE SUITE IS NOT CLEAN: five failures, all PRE-DATING 2026-09-09 (ONE CLOSED 2026-09-10, four left)
...
| `verify-accounts` | 124/1 | **CLOSED 2026-09-10, 125/0** | S10 was a STALE ALLOW-LIST, not a breach. The route queries `refm_asset_types` alone and resolves the caller through `resolveAccountId`, so it reads no `users` row: the check is a coarse string match on `account_id` and its own comment says each step amends the list consciously. Mig 241 added the cost-catalog route for exactly this reason; mig 242 added this route and did not. Admitted by name, with the reason. |
