...
- **Training certificates page silently empty for every student**: `GET /api/training/certificates` selects `users.full_name` (the column is `name`) and `certificates.certificate_number` (005's columns never landed; live `certificates` is 002's shape). The first miss fails silently and returns `[]`, shielding the second from 500ing. Decide whether this page should read `student_certificates` (the real cert system) or be retired; do not just patch the column names without that decision.
- **Migration 222 (password_reset_tokens) PENDING manual apply**: until then password reset answers an honest 503. Apply, then run a live end-to-end (request, email, token, reset, sign-in).
