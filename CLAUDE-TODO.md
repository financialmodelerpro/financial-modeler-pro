...
**▶ NEXT = Phase 3 (Revenue + Cost of Sales).** Then, one or two sheets per commit (each reconciled): 4 Opex, 5 Financing (debt/IDC iterative/sweep/equity), 6 Fixed Assets + Depreciation, 7 Statements (P&L/CF/BS), 8 Returns (Excel IRR/NPV on the streams), 9 final Checks. Reuse the `lib/reports/` builders (capex/cos/opex/financing/m4) for the cached `result` values so Excel ties to the platform + PDF by construction. Phase 3 specifics: add revenue inputs per asset to Assumptions; a Revenue sheet, per-asset Sell (velocity -> sale value -> cash/recognition vintage via SUMPRODUCT), Hospitality (ADR×occupancy×keys×365), Lease (rent×area×occupancy); Cost of Sales matched to recognition (reuse cosReports.ts cached results). Keep the look (no gridlines, navy bars, bordered cards, zebra fills, FAST colours).
