...
- **Training certificates page: CLOSED 2026-08-30g by RETIREMENT.** The legacy endpoint, its pages and the whole `/api/admin/assessments` family are deleted; the admin course editor survives lessons-only; the five legacy tables are deprecated in place (mig 223, PENDING apply). One certificate system remains: `student_certificates` + the `certificates` storage bucket. Pinned by `verify-legacy-training-retirement` 22 (its scan distinguishes table reads from bucket reads, TRAPS 2.10).
