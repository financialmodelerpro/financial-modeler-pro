...
| `144_admin_cleanup.sql` | Companion to the multi-phase admin cleanup (commits `4a5abe3` / `a000fbd` / `ee959ad` / `d8405e5`). Drops 5 tables whose admin UIs + APIs were just removed — all `DROP TABLE IF EXISTS … CASCADE` so dependent FKs/views/triggers go with them and re-runs are no-ops. **Permissions trio (mig 006):** `user_permissions`, `plan_permissions`, `features_registry` — backed the deleted `/admin/permissions` + `/admin/overrides` + `/admin/plans` pages, `/api/permissions` + `/api/admin/permissions`, `src/lib/shared/permissions.ts`, `src/hooks/useSubscription.ts`, and the 486-line `PermissionsManager.tsx`. REFM `canAccess()` now stubs to `false` (premium features locked pre-launch); a real plan-based gating system gets reintroduced when paid tiers go live. **Pricing extensions (mig 014):** `pricing_features`, `pricing_modules` — backed the deleted Pricing Features + Module Access tabs. The Modeling Hub Modules admin (`/admin/modules`, separate `modeling_modules` table) is unaffected. Apply manually via Supabase dashboard. |
